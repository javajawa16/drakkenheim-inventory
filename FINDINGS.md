...
### 2026-06-24 (run 58) — Sections audited: 6 (Code.js 3900–end + the `apiSetItemQuantity`/quick-adjust helpers it feeds)

**Stories traced (happy → failure-at-step → navigate-away → friction, with execution-trace + state-machine on each write path):**
- **Quick-adjust currency/delerium — "set" mode** (tap card → quick-edit sheet → choose "set" → enter amount → Confirm). Client `confirmQuickEdit` (Index.html:7559) → `apiSetItemQuantity` (Code.js:3830). Re-traced the full server path (auth → `tryLock(10000)` → validate id/qty/note → `getInventoryRowObjectById_` → `classifyQuickEdit_` → optional delerium rename → `writeInventoryRow_` → conditional `appendResourceLedger_` → `auditWrite_` → `bumpSync_`), and the client `finishSuccess`/`fail` handlers (7575/7604). Found the rename BUG below. Lock acquired after auth and released in `finally` on every path (success, validation throw, auth-failure). Client `finishSuccess` re-enables the Confirm button and clears `quickEditInFlight` on both ok and `!ok` branches, and the `capturedItemId` guard (7598) correctly suppresses the success paint/close when the panel has been re-pointed at another item during the round-trip — navigate-away is handled. The `_inFlightWrites`-not-bumped gap for this handler (run 20, Index.html:7145) and the `quickEditInFlight`-never-reset stuck-state (run 20, 7126) remain the governing risks for the failure/background scenarios; both already tracked.
- **Helpers** `normalizeForClient_` (3941), `ensureInventoryHeaders_` (3956), `findInventoryRowById_` (4033), `writeInventoryRow_` (1855), `getInventoryRowObjectById_` (1842) re-read in service of the trace. `findInventoryRowById_` correctly text-finds against column 1 (`Inventory ID` is `INVENTORY_HEADERS[0]`), literal match (no regex), `matchEntireCell(true)` — clean. `writeInventoryRow_`/`ensureInventoryHeaders_` column counts are consistent (headers re-read from `getLastColumn()` after `ensureInventoryHeaders_` runs). `fillMissingInventoryRarity_` (3973) produced the RISK below.

#### BUG · Code.js:3859,3749 · "Set"/adjust quick-edit still renames a *custom-named* delerium row to "Delerium Chip" — run-20 fix was incomplete and regressed into `set` mode

**Story: Quick-adjust currency/delerium (set AND add/remove).** Run 20 logged the rename-to-`chip` corruption (`#### BUG · Code.js:3773`, marked **FIXED**) and noted at the time that the asymmetry "`set` mode … never sends `size` and never renames — only `add`/`remove` corrupt" was what confirmed it unintended. Two things have changed since, and together they re-open the bug:

1. **The fix was partial.** The applied fix was client-side: `populateQuickSize` now does `if (currentSize) select.value = currentSize` (Index.html:7503). The server sends `currentSize: normalizeDeleriumSize_(found.rowObj['Item'])` (Code.js:3691). For a **canonical** row ("Delerium Geode") this normalizes to `'geode'`, which IS one of the `<option>` values, so the dropdown pre-selects correctly and the server's `rowObj['Item'] = 'Delerium Geode'` rename is a harmless no-op. But for a **custom-named delerium-category** row — e.g. **"Aqua Delerium"** (a real campaign item; README:51 shows "2× Aqua Delerium") whose `Category === 'delerium'` — `normalizeDeleriumSize_('Aqua Delerium')` returns the raw `'aqua delerium'` (not in the map, 1136), which is **not** one of the `<option>` values, so `select.value = 'aqua delerium'` silently fails and the dropdown stays on its **first** option, `'chip'` (Code.js:43). `classifyQuickEdit_` returns `'delerium crystal'` via the `category === 'delerium'` branch (1934), so the size field is `.active` and `confirmQuickEdit` sends `size: 'chip'`.

2. **`set` mode now renames too.** A later fix ("`#### BUG · Code.js:3860 · "Set" quick-edit mode silently drops the delerium size selection`", marked FIXED) added a `size` parameter and a delerium rename branch to `apiSetItemQuantity` (3859–3864): `if (size && DELERIUM_SIZE_VALUES.includes(size)) rowObj['Item'] = 'Delerium ' + size…`. `apiAdjustInventory` has the identical branch (3749–3752). So **both** quick-adjust paths now rename.

**Net effect:** doing a plain quantity **set** (or add/remove) on an "Aqua Delerium" row renames it to **"Delerium Chip"**, destroying the item's name. It compounds exactly as run 20 described: the ledger `subtype` is derived from the renamed item (`normalizeDeleriumSize_(rowObj['Item'])`, 3868/3880) so the RESOURCE_LEDGER records size `chip`, and the row's quantity now collides/merges with any real "Delerium Chip" stock on subsequent reads. The user gets a "Quantity updated." / "Inventory adjusted." success with no hint the name was changed.

**Fix:** make the rename a no-op unless the size actually changed AND the name was already canonical — i.e. only rewrite `Item` when `normalizeDeleriumSize_(found.rowObj['Item'])` is itself a canonical `DELERIUM_SIZE_VALUES` member that differs from the submitted `size`. Equivalently (and matching run 20's "better" recommendation): drop the size field from quick-adjust of an *existing* delerium row entirely — the row already has a fixed identity; a quantity tweak should never re-classify or rename it. A genuine "convert size" feature, if wanted, should be an explicit separate action.

#### RISK · Code.js:4029 · `fillMissingInventoryRarity_` performs an unlocked, full-column `setValues` write inside the lock-free `apiGetInventory` read

**Story: Cross-cutting — collaborative sync interference / any inventory load.** `apiGetInventory` (2103) takes **no** `LockService` lock (it is a read), but it calls `fillMissingInventoryRarity_` (2114), which — when any inventory row has a `Library Item ID` whose `Rarity` is blank and a matching rarity exists in `EQUIPMENT_LIBRARY_CLEAN` — writes the **entire** Rarity column back with `inventorySheet.getRange(2, rarityCol, numRows, 1).setValues(rarities)` (4029). Two concurrency hazards, because no lock guards the read→write window while other endpoints (`apiAdjustInventory`, `apiSetItemQuantity`, `apiUpdateInventory`, `apiDeleteInventory`) hold only the *document* lock that this read does not participate in:

1. **Row-shift misalignment.** `numRows` and the `combined` snapshot are captured at function entry. If a concurrent locked writer deletes a row (`apiDeleteInventory` → `deleteRow`) between the snapshot and the `setValues`, the sheet now has `numRows-1` data rows but the write still pushes `numRows` rows starting at row 2 → every rarity from the deleted point down is written one row too low (wrong item) and the trailing write lands in a now-empty row.
2. **Clobber of a concurrent rarity edit.** The write rewrites *all* rarity cells (not just the filled-in ones), using values read into `combined`. If `apiUpdateInventory` commits a Rarity change in the same window, this backfill silently reverts it to the stale snapshot value.

The blast radius is bounded — it only fires when `changed` is true (a library-matched item with a blank rarity), it is self-limiting (once written, that cell is no longer blank so it stops firing), and the resulting bad write is invisible to other clients because `apiGetInventory` does not `bumpSync_`. But the window is real and recurs on every load/tab-switch/sync-reload until each blank-rarity item is backfilled. **Fix:** either guard the backfill with the same document lock (acquire/`tryLock`/release in `finally`) so it serializes with writers, or write only the specific changed cells individually after re-finding them by `Inventory ID` rather than rewriting the whole column off a stale row snapshot.

#### Note · Code.js:3830 · `apiSetItemQuantity` lock/audit/ledger discipline is otherwise clean; `findInventoryRowById_` + header helpers trace correct

**Stories: Quick-adjust currency/delerium (set mode); Quick-adjust currency (no size field).** Outside the rename BUG, `apiSetItemQuantity` is disciplined: `tryLock(10000)` after auth, released in `finally` on all paths; both SUCCESS and FAILED audit rows written; the ledger entry is created only when `quickType` is currency/delerium AND `qty !== oldQty` (3872), with a correctly-signed `delta` and a sanitized `ledgerEntry` returned for the client's optimistic prepend. The currency path (size field inactive) is unaffected by the delerium rename. `findInventoryRowById_` (4033), `getInventoryRowObjectById_` (1842), `writeInventoryRow_` (1855), `ensureInventoryHeaders_` (3956), and `normalizeForClient_` (3941) were re-verified and trace correctly. Pre-existing known issues touched by this story but already tracked: the second-boundary timestamp mismatch between the client's `_ts` (computed pre-append) and `appendResourceLedger_`'s `new Date()` (1991), and `appendResourceLedger_` swallowing its own errors (README Known TODOs).
