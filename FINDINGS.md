...
### 2026-07-24 (run 76) — Sections audited: 12 (Index.html 7501–end: the "quick-edit, add item flow, custom item, form handling" slice. The range holds the description-sheet Sell/Remove confirm tail `_confirmDescActionSell_`/`_confirmDescActionRemove_` 7473/7547 and the dead `deleteSelectedInventoryFromDescription` 7622 (never wired); the **quick-edit currency/delerium** layer `getQuickEditType` 7629 / `quickPrefix` 7648 / `openQuickEditPanel` 7656 / `populateQuickSize` 7728 / `onQuickSizeChange` 7743 / `setQuickEditorOpen` 7764 / `closeQuickEditPanel` 7782 / `clearQuickEditFields` 7791 / `openFullEditorFromQuick` 7799 / `confirmQuickEdit` 7809; the **full inventory editor** `syncInventoryEditorFieldsFromRow` 7892 / `setInventoryEditorOpen` 7927 / `getInventoryEditorValues` 7944 / `getEditStatusElement` 7973 / `closeInventoryPanels` 7977 / `saveInventoryEdits` 7987 / `updateInventoryRowLocally` 8023 / `updateInventoryRowFromServer` 8037; the **swipe-delete** `getDeleteTargetRowByRepresentativeId` 8050 / `handleInventoryDeleteActionById` 8061 / `deleteSelectedInventory` 8096; the **Add flow** `loadEquipmentIndex` 8196 / `searchEquipment` 8291 / `getQuickAddMatches` 8333 / `renderEquipmentResults` 8344 / `selectEquipmentResult` 8386 / `fillAddFormFromEquipment` 8406 / `setScrollMode_`/`handleScrollSpellInput`/`handleCustomItemNameInput` 8493/8500/8508 / `focusPrimaryAddField` 8519 / `changeSelectedItem` 8554 / `startCustomFromSearch`/`startCustomItem`/`customizeSelectedItem` 8570/8575/8604 / `updateAddFlow`/`updateAddStepper_` 8642/8667 / `loadSelectedDescription` 8688 / `addInventoryItem` 8734 / `clearAddForm` 8904; and the **global tail** — `escapeHtml`/`escapeJsString` 8961/8970, the delegated `click`/`focusin`/`resize`/`visibilitychange` listeners 8977/8984/8990/9005, and the boot block 9016. Read out to `cacheInventoryRows`/`primeInventoryCacheAfterAdd`/`getCachedInventoryPayload`/`bustInventoryCache` 3232/3243/3175/3152, `gasCall`/`drainPendingForeignReload_` 3035/3046, `findDuplicateInventoryCandidate`/`showCombineChoice`/`confirmCombineInventoryItem` 3696/3705/3729, `renderInventory`/`renderInventoryDashboard`/`renderInventoryRowCard` 4043/4146/4103, `setCommandMode` 3515, `updateAddFormInteractionState`/`handleSearchInput` 3864/3888, and the server endpoints `apiAdjustInventory`/`apiSetItemQuantity` Code.js 3722/3845.)

> **Stories traced** (happy → failure-at-step → navigate-away → friction, execution-trace + state-machine on every write path and UI component): **Quick-adjust currency/delerium** (`openInventoryPrimaryAction` → `getQuickEditType` 7629 → `openQuickEditPanel` 7656 optimistic-local `selectedQuickEdit` + async `apiGetCurrencyQuickEdit` refresh with the `selectedQuickEdit.itemId !== row['Inventory ID']` staleness guard 7698 → `confirmQuickEdit` 7809 via `apiSetItemQuantity`/`apiAdjustInventory`; `onQuickSizeChange` 7743 reclassify-confirm); **Edit inventory item** (swipe → Edit → `syncInventoryEditorFieldsFromRow` 7892 + `editQty`/`sheetEditQty` readOnly reset 7921 → `saveInventoryEdits` 7987 → `apiUpdateInventory` → `updateInventoryRowLocally` 8023, non-optimistic, button re-enable on both branches); **Delete inventory item** (two-tap arm `handleInventoryDeleteActionById` 8061 → `deleteSelectedInventory` 8096 qty>1 confirm gate 8111, optimistic filter/decrement + `previousRows` revert, `apiDeleteInventory`/`apiAdjustInventory` decrement 8185); **Add library item** (`searchEquipment` 8291 → `selectEquipmentResult` 8386 → `loadSelectedDescription` 8688 → `fillAddFormFromEquipment` 8406 → `addInventoryItem` 8734 optimistic `_opt_` row 8801 + `clearAddForm` + `gasCall('apiAddInventory')` → `onAddSuccess` 8814 removes optId, `primeInventoryCacheAfterAdd` 3243, combine suggestion `findDuplicateInventoryCandidate`/`showCombineChoice` 8850); **Add custom item** (`startCustomFromSearch`/`startCustomItem`/`customizeSelectedItem` → `apiAddCustomInventory`); **Combine duplicate** (`confirmCombineInventoryItem` 3729); plus the cross-cutting **Collaborative sync interference** / **Tab-switch in-flight** / **iOS background-foreground** stories where they cross these writes via `gasCall`/`_inFlightWrites`/`drainPendingForeignReload_` 3035–3051 and the `cacheInventoryRows` in-flight guard 3232, and the **resize/visibilitychange** global listeners 8990/9005. Per protocol I re-verified prior in-range findings and did **not** re-report them: the **`quickEditInFlight` never-resets-on-dropped-callback** RISK (run-63/64 twin, FINDINGS:3481; `closeQuickEditPanel` 7782 still deliberately keeps the flag, 7784–7786, no `visibilitychange` reset — **still open**); the run-51 **delerium size-merge** server RISK (`apiAdjustInventory` still renames the row keeping `oldQty+delta`, Code.js 3757–3768 — **still open**, and `onQuickSizeChange` 7743's reclassify-confirm is the client half); the run-65 **`Cannot remove …` error-allowlist** BUG (Code.js 3753 still maps to "Request failed." on a too-large quick-adjust remove — **still open**; `confirmQuickEdit` 7824 pre-guards only `amount >= 0`/`Number.isFinite`, never `amount <= held`); the run-51 **health-potions-never-quick-edited** IDEA (`getQuickEditType` 7629 still returns `''` for potions — **still open**); the run-65 **blank-Value-GP override** BUG (`apiAddInventory` still rewrites a deliberately-blanked value to the library default while add/edit/custom honor blank — **still open**); the run-52 fixes still landed — holder-wipe (`fillAddFormFromEquipment` blanks `#holder` only on initial fill 8443), failed-add clobber (both `onAddSuccess` !ok 8820 and `onAddFail` 8862 guard the restore on `selectedEquipment === null`), `_opt_`-row-not-cached-at-insert (8802 comment, no cache call), and the run-58 Cancel-mid-flight double-apply fix (7784). **Verified NOT a bug** (each checked because it looked like a stuck/lost state): (a) `cacheInventoryRows(inventoryRows)` single-arg calls (`saveInventoryEdits` 8011, `primeInventoryCacheAfterAdd` 3254) are safe — the omitted 2nd arg falls back to the global `inventoryResourceLedger` (3237); (b) a quick-adjust that drives a currency/delerium row to `Qty 0` leaves a `Qty:0` row (server never deletes at 0, Code.js 3753/3872) and `renderInventory` never filters 0-qty rows (4048–4071), but gold/delerium rows are `isDashboardResourceRow_`-excluded from the card list and only aggregated into the dashboard stat totals (4146–4160), so a 0-qty resource row shows no phantom card — it just contributes 0; (c) `deleteSelectedInventoryFromDescription` 7622 is **dead** (no `onclick` — the two `deleteSelectedInventory()` buttons are the desktop/mobile editor Delete 2452/2670 and Remove-from-description now routes through `_confirmDescActionRemove_`), so its would-be "confirm-button appended to a closed sheet's hidden status" defect is unreachable; (d) `addFormInteractionPrimed` (3092) is written in six places but **never read** — dead state, no behavioral effect, not reported per the no-dead-code rule. **One new finding.**

#### RISK · Index.html:8990–9002 · The `window` `resize` handler force-closes the inventory **Edit** sheet on any >150px viewport-height change, silently discarding all in-progress unsaved edits — deterministically on device rotation, and (on Android/webviews where the soft keyboard resizes `window.innerHeight`) the moment the user focuses a field to type

**Story: Edit inventory item — the navigate-away / viewport-change scenario (step c) on the full editor; state-machine on the open-editor → viewport-resize transition.** The only `resize` listener in the app (8990) computes `_heightDelta = |innerHeight − lastHeight|` and, when it exceeds 150 px, calls `setInventoryEditorOpen(false)` and `closeDescriptionSheet()` (8995–8998):

```
8990  window.addEventListener('resize', () => {
8992    const _heightDelta = Math.abs(_newHeight - _resizeLastHeight);
8993    _resizeLastHeight = _newHeight;
8994    updatePhoneClass();
8995    if (_heightDelta > 150) {
8996      setInventoryEditorOpen(false);   // ← closes the Edit sheet
8997      closeDescriptionSheet();
8998    }
```

`setInventoryEditorOpen(false)` (7927) merely strips the `.active`/`display` off `#inventorySheet`; it does **not** persist the typed field values. The Edit sheet's inputs (`sheetEditItem` 2619, `sheetEditNotes` 2624, `sheetEditValueGp` 2654, plus Qty/Holder/Category/Rarity/Faction) keep their DOM values while hidden, but the only way back into the editor is tapping the card → `selectInventoryItem` → `syncInventoryEditorFieldsFromRow(row)` (7892), which **overwrites every field from the stored row**. So once the resize fires, the user's un-saved edits are irrecoverable — and unlike the Add flow, which snapshots `payloadSnapshot`/`selectedSnapshot` and restores the form on failure (8766/8820/8862), the Edit path has **no** snapshot/restore.

Two triggers, both realistic:

1. **Device rotation — platform-independent, deterministic.** Portrait↔landscape on a phone swaps innerHeight by hundreds of px (e.g. ~844↔~390, delta ~454 > 150). A player who opens Edit, starts changing the notes/value, then rotates their phone (a completely ordinary thing to do mid-task) has the sheet vanish and every typed change discarded with no warning, no confirm, no "unsaved changes" prompt.

2. **Soft keyboard on Android/some webviews — functional break, not just data loss.** On Android Chrome/WebView the software keyboard shrinks the layout viewport, firing a `resize` with `innerHeight` down ~250–350 px (> 150). Focusing the Notes textarea (or any field) to type would immediately trip 8995 and close the sheet — making the Edit sheet effectively **unusable** on those devices. (This survives in production because the README's primary target is the **iOS** GAS webview, where the keyboard overlays without changing `window.innerHeight` — only `visualViewport` moves — so on the dev's iOS device the handler never fires on focus. The app has no `visualViewport` listener and the `--keyboard-offset` var is never JS-updated, confirmed dead by runs 58/60, so this is genuinely untested on keyboard-resizing platforms.)

The handler's `> 150` threshold and adjacent `updatePhoneClass()` show its intent is to react to genuine orientation/viewport changes — but it applies a read-only-sheet remedy (fine for the description sheet, which has no inputs) to a **data-entry** sheet. The asymmetry confirms it isn't a principled "close all modals on resize" policy: it force-closes only the Edit and description sheets, while the quick-edit sheet, add form, combine sheet, note form, and gold/delerium sheets all stay open across the same resize. Fix options: (a) exclude `#inventorySheet` from the resize auto-close (rotation with a slightly-imperfect layout is far less harmful than silent data loss); (b) if it must close, snapshot the editor fields the way `addInventoryItem` does and re-hydrate on reopen; or (c) gate the auto-close on an actual orientation change (`matchMedia('(orientation: …)')` / comparing width-and-height flips) rather than a raw height delta, so a keyboard-only resize never trips it.

### 2026-07-23 (run 75) — Sections audited: 11 (Index.html 6001–7500: the "inventory groups, description sheet, sell batch" slice. The range holds the inventory-group classifier layer `getDeleriumBreakout`/`buildInventoryGroups`/`isDashboardResourceRow_`/`getInventoryGroupKey`/`isAccessoryItem_`/`isTreasureItem_`/`renderInventoryGroup`/`getInventoryRollupKey`/`rollupInventoryRows`/`toggleInventoryGroup` 6520/6554/6574/6583/6615/6627/6638/6654/6662/6702, the dashboard inline pay `payResource`/`undoResourcePay` 6709/6786, the card entry points `openInventoryIndexFromGesture`/`getInventoryIndexById`/`setSelectedInventoryCardById`/`handleInventoryCardClickById`/`openInventoryPrimaryActionById`/`openInventoryPrimaryAction`/`selectInventoryItem` 6874/6880/6892/6898/6903/6911/6927, the **Item description sheet** `openInventoryDescriptionById`/`openInventoryDescription`/`getDescRemoveTotalQty_`/`updateDescRemoveRow_`/`stepDescRemoveQty`/`closeDescriptionSheet` 6968/6974/7052/7065/7075/7155, and the **description action sub-sheet** `openDescActionSheet`/`closeDescActionSheet`/`renderDescActionBody_`/`stepDescAction`/`selectDescGiveTarget`/`confirmDescAction`/`_confirmDescActionGive_`/`_confirmDescActionAdd_`/`_confirmDescActionSell_`/`_confirmDescActionRemove_` 7162/7187/7192/7251/7262/7270/7279/7380/7473/7547. Read out to the render/cache plumbing `renderInventoryRowCard` 4103, `renderStatBlock_`, `cacheInventoryRows`/`primeInventoryCacheAfterAdd` 3232/3243, `gasCall`/`drainPendingForeignReload_` 3035/3046, and the two-tap sell-confirm helpers `_requireSellConfirm`/`_clearSellConfirm`/`_armSellBtn`/`_disarmSellBtn` 3073/3067/3052/3060.)

> **Stories traced** (happy → failure-at-step → navigate-away → friction, execution-trace + state-machine on every write path and UI component): **View item details** (tap card → `handleInventoryCardClickById` 6898 → `openInventoryPrimaryAction` 6911 → `openInventoryDescription` 6974 → async `apiGetEquipmentItem` with the `capturedId`/`selectedInventory` staleness guard 7023–7037, `itemCache` only-cache-on-ok 7028); **Give item to character** (`openDescActionSheet('give')` 7162 → `selectDescGiveTarget` 7262 → `_confirmDescActionGive_` 7279 via `apiGiveInventoryBatch`, optimistic recipient row + reload-on-success 7363); **Sell item** (`openDescActionSheet('sell')` → two-tap `_requireSellConfirm` 3073 → `_confirmDescActionSell_` 7473 via `apiSellInventoryBatch`, no-cache-until-settled note 7517); **Remove item** (`_confirmDescActionRemove_` 7547, goldAmount=0 `apiSellInventoryBatch`); **Add library/custom item** re-add-from-description (`_confirmDescActionAdd_` 7380, `existingMatch` 5-tuple in-place increment vs server auto-combine); the FIFO-drain steppers (`getDescRemoveTotalQty_` 7052, `stepDescAction` 7251, `renderDescActionBody_` 7192); plus the cross-cutting **Collaborative sync interference** / **Tab-switch in-flight** / **iOS background-foreground** stories across the `gasCall`/`_inFlightWrites`/`drainPendingForeignReload_` layer (3035–3051) and the read-side group/rollup helpers (`buildInventoryGroups` 6554 / `getInventoryGroupKey` 6583 / `rollupInventoryRows` 6662). Per protocol I re-verified prior in-range findings and did **not** re-report them: run-11/64's **`isMultiple` dead-branch DM cross-holder rollup** BUG (FINDINGS:4001 — re-confirmed **the stepper half is FIXED**: `isMultiple` is now computed as `holders.size > 1` from the row set in `getDescRemoveTotalQty_` 7058, `_confirmDescActionSell_` 7486, `_confirmDescActionRemove_` 7555, and `_confirmDescActionGive_` 7288, so the Remove/Sell/Give steppers now drain the full cross-holder total — **but the description-sheet title was left on the old rep-holder-only computation**, which is the one new finding below); the description-Sell/Remove `descActionInFlight` re-entry guard (present at 7474/7548/7381/7280); the `apiSellInventoryBatch`/`apiGiveInventoryBatch` gold-before-delete ordering (run-19, confirmed server-side); the "don't cache until `_inFlightWrites` cycles" note on `_confirmDescActionSell_`/`_confirmDescActionRemove_` (7517/7588 — the optimistic sold-down rows are deliberately **not** persisted before `gasCall`, so an unconfirmed sell/remove can't leak to localStorage; the success/fail handlers re-cache once the write has cycled — the correct counter-pattern to the still-open `sellDelerium` FINDINGS:204 leak). **Verified NOT a bug** (each checked because it looked like a stuck/lost/incorrect state): (a) the legacy pinned-remove machinery `confirmDescRemove` 7081 / `stepDescRemoveQty` 7075 / `updateDescRemoveRow_` 7065 targets `descRemoveMinus`/`descRemovePlus`/`descRemoveQtyDisplay` ids that exist only inside these functions and are never rendered by the current sub-sheet (`renderDescActionBody_` builds `descActionMinus`/`descActionPlus`/`descActionQtyDisplay`), and `openSellItemSheet` 5583 / `confirmSellItem` 5604 / `openGiveItemSheet` 5553 / `giveItemToCharacter` 6043 have no `onclick` wiring — all superseded by `openDescActionSheet`/`confirmDescAction`, so **dead, not re-counted**. (b) the `Add` sub-sheet mode's `stepperHtml(descActionQty, 99, …)` cap of 99 (7214) is intentional (adding copies, no stock ceiling) not a stepper-max bug. (c) a foreign `loadInventory(true)` firing while the description sheet is open (no in-flight write, so not deferred) replaces `inventoryRows` under the open sheet, but every confirm handler re-derives `allForKey`/`matchingRows` from the *current* `inventoryRows` by rollup key and `return`s on an empty match, so a race can only no-op the Confirm, never sell/remove the wrong rows or crash — the stale `selectedInventory` object is used only for its rollup-key/holder identity. (d) `_confirmDescActionGive_`'s success handler drops the optimistic `_opt_give_` row and calls `bustInventoryCache()`+`loadInventory(true)` rather than caching the optimistic state (7363–7367), because the server-assigned recipient row ID is unpredictable; the brief cache-less window self-heals on reload — deliberate, not a loss. **One new finding.**

#### BUG · Index.html:6988–6991 · The FINDINGS:4001 multi-holder-rollup fix updated the Remove/Sell/Give stepper max (`getDescRemoveTotalQty_` → `holders.size > 1`) but left the description-sheet **title** on the old rep-holder-only sum, so a DM tapping a `5× Potion (Multiple)` card now sees a title that reads `2× Potion` while the stepper still lets them drain all 5 — the title silently under-reports and disagrees with both the card and the operation it drives

**Story: View item details + Remove/Sell/Give item — happy path in DM scope on a rollup that spans two holders; state-machine on the description-sheet title vs. stepper-max transition.** `rollupInventoryRows` (6662) merges inventory rows by `Item|Category|Rarity` (holder excluded, 6654) and, when the merged rows span different holders, sets `Holder = 'Multiple'` (6687) and sums `Qty` across them. So in **DM scope** — the only scope where a rollup spans holders (Party scope filters to empty-holder rows, player scope to one holder) — a potion held by Alice(2) and Bob(3) renders as one `5× Potion · Multiple` card (`renderInventoryRowCard` 4110/4118 read the rolled-up `Qty`/`Holder`).

Tapping that card routes through `handleInventoryCardClickById(representativeId)` 6898 → `openInventoryPrimaryAction(index)` 6911, where `index = getInventoryIndexById(representativeId)` resolves into the **raw** `inventoryRows` array — so `selectedInventory` is Alice's raw single-holder row (`Holder = 'Alice'`, `Qty = 2`), never the rolled-up object (`_rollupKey`/`Holder:'Multiple'` are not carried in). `openInventoryDescription` (6974) then computes the two numbers from **different holder scopes**:

```
6986  const _repHolder = String(row['Holder'] || '').trim();          // 'Alice'
6988  const _titleQty = (inventoryRows || [])
6989    .filter(r => getInventoryRollupKey(r) === _rKey && String(r['Holder'] || '').trim() === _repHolder)
6990    .reduce((sum, r) => sum + toNumber(r['Qty'] || 1), 0) || 1;    // → 2 (Alice only)
6991  document.getElementById('descriptionTitle').textContent = (_titleQty > 1 ? _titleQty + '×… ' : '') + …;
```

versus the stepper ceiling that the FINDINGS:4001 fix rewrote to cover the whole rollup:

```
7052  function getDescRemoveTotalQty_() {
7056    const allForKey = (inventoryRows||[]).filter(r => getInventoryRollupKey(r) === rollupKey); // Alice+Bob
7057    const holders   = new Set(allForKey.map(r => String(r['Holder']||'').trim()));             // {Alice,Bob}
7058    const isMultiple = holders.size > 1;                                                        // true
7059    const total = allForKey.filter(r => isMultiple || …).reduce(sum) ;                          // → 5
```

The title filters by `_repHolder` (Alice → **2**); `getDescRemoveTotalQty_` returns the cross-holder total (**5**), and `stepDescAction` (7251) / `renderDescActionBody_` (7192) use that 5 as the Remove/Sell/Give stepper max. **Net result in DM scope:** the card says `5`, the description title says `2×`, and the Remove/Sell/Give stepper lets the DM step up to `5` — draining Bob's 3 units via the FIFO loop in `_confirmDescActionRemove_`/`_confirmDescActionSell_`/`_confirmDescActionGive_` (they iterate `matchingRows` = all-holder rows sorted by Date Added, 7289/7487/7556) — all while the on-screen title claims only 2 exist.

This is the residual half of FINDINGS:4001. That finding offered two fix options: (a) carry the tapped rollup's `_rollupKey`/holder context into `selectedInventory` so the stepper covers all holders, or (b) make the **title** reflect the rep-holder subset so card and sheet agree. The implementer applied a variant of (a) to `getDescRemoveTotalQty_` and the three confirm functions — correctly restoring the ability to remove/sell/give the full stack (the original "cannot remove Bob's 2 units" defect is genuinely fixed) — but did **not** touch the title's `_titleQty` computation, which still uses option-(b)'s rep-holder scoping. So the title now disagrees with *both* the card (5) and the now-correct stepper (5). It is strictly a display defect — the write itself operates on the correct full set — but it is a misleading count on exactly the destructive path (Remove/Sell/Give) where a DM most needs to trust the number: the title says "2" while a full-stepper Remove silently deletes another player's items. Fix: compute `_titleQty` with the same `holders.size > 1 ? all-holder-total : rep-holder-total` rule as `getDescRemoveTotalQty_` (or just reuse `getDescRemoveTotalQty_()` for the title), so the title, the card, and the stepper max all show 5.

### 2026-07-22 (run 74) — Sections audited: 10 (Index.html 4501–6000: the "gold/delerium UI, sync" slice. The range holds the identity boot/apply layer read out for cross-cutting traces — `readCachedIdentity`/`cacheIdentity`/`applyIdentity`/`loadMyIdentity`/`loadFallbackCharacterIdentity`/`showIdentitySheet`/`initIdentityPicker`/`confirmIdentity` 4660/4673/4687/4727/4734/4756/4783/4795 — the collaborative-sync poll `pollSync`/`startSyncPoll`/`stopSyncPoll` 4179/4210/4215 and its `gasCall`/`drainPendingForeignReload_` plumbing 3035/3046, the **Gold** sheet layer `openGoldSheet`/`closeGoldSheet`/`renderGoldSheetBody`/`renderGoldSheetButtons`/`setGoldSheetScope`/`positionGoldScopeIndicator` 4831/5344/5355/5395/5853/4862 with its write paths `receivedGold` 5434 / `openPayReasonSheet`+`confirmPayWithReason` 5515/6099 / `splitGold` 6208 / `undoGoldPay`+`undoResourcePay` 5428/6786 and the inline-dashboard `payResource`/`renderResourceBreakout` 6709/6271, the **Delerium** sheet layer `openDeleriumSheet`/`closeDeleriumSheet`/`renderDeleriumSheetBody`/`renderDeleriumSheetActions`/`adjustDeleriumSell`/`adjustDeleriumReceive`/`updateDeleriumButtonStates`/`refreshDeleriumStateFromInventory_` 4926/4935/4944/4875/5024/5071/5091/5214 with `receiveDelerium` 5112 / `sellDelerium` 5225, the shared **ledger note-edit** `renderResourceLedger`/`getLedgerFlow_`/`selectLedgerRowForEdit`/`cancelLedgerEdit`/`updateLedgerNoteFromBottom` 6356/6323/6405/6436/6453, the resource classifiers `isGoldItem_`/`getGoldBreakout`/`isDeleriumCrystalRow_`/`getDeleriumBreakout` 6491/6497/6514/6520, the **Give/Sell item** sheets `openGiveItemSheet`/`giveItemToCharacter`/`openSellItemSheet`/`confirmSellItem` 5553/6043/5583/5604, and the treasurer **Sell Items batch** `openSellBatchSheet`/`buildSellBatchRollups`/`renderSellBatchBody`/`stepSellBatchQty`/`confirmSellBatch`/`copyPartyPoolInventory` 5680/5695/5738/5804/5969/5875. Read out to `loadInventory`/`cacheInventoryRows`/`getCachedInventoryPayload`/`primeInventoryCacheAfterAdd` 3917/3232/3175/3243 and the `visibilitychange` resume 9005.)

> **Stories traced** (happy → failure-at-step → navigate-away → friction, execution-trace + state-machine on every write path): **Receive gold** (`receivedGold` 5434), **Pay gold** (`openPayReasonSheet` 5515 → `confirmPayWithReason` 6099 via `apiDepleteResource`/`apiSendGoldToMember`), **Split gold evenly** (`splitGold` 6208), **Undo last pay** (`undoGoldPay` 5428 → `undoResourcePay` 6786 purchase/member branches), **Edit ledger note** both resources (`selectLedgerRowForEdit` 6405, `cancelLedgerEdit` 6436, `updateLedgerNoteFromBottom` 6453, `renderResourceLedger` 6356); **Receive crystals** (`receiveDelerium` 5112) and **Sell crystals** (`sellDelerium` 5225 incl. the 0-gp inline confirm) with the counter state machine (`adjustDeleriumSell` 5024 / `adjustDeleriumReceive` 5071 / `updateDeleriumButtonStates` 5091 / `refreshDeleriumStateFromInventory_` 5214); **Give item to character** (`openGiveItemSheet` 5553 → `giveItemToCharacter` 6043), **Sell item** (`openSellItemSheet` 5583 → `confirmSellItem` 5604), the treasurer **Sell Items batch** (`openSellBatchSheet` 5680 → `confirmSellBatch` 5969, plus `copyPartyPoolInventory` 5875); and the cross-cutting **Collaborative sync interference** / **Tab-switch in-flight** / **iOS background-foreground** stories where they cross the open gold/delerium sheets (`gasCall`/`drainPendingForeignReload_` 3035/3046, `pollSync` 4179, `loadInventory` 3917 whose success handler at 3982 still calls only `renderInventory()`). This section was last swept in full by run 62 (FINDINGS:403) and its write endpoints re-driven from the server side by run 68 (FINDINGS:200); per protocol I re-verified prior in-range findings and did **not** re-report them: run-62's **receivedGold `!res.ok` branch dropped `renderInventory()`** BUG (FINDINGS:417 — re-confirmed **FIXED**: `renderInventory()` now present in the gold error branch at 5491); run-62's **open Gold/Delerium sheet shows a stale ledger + header total after a foreign `loadInventory(true)`** RISK (FINDINGS:407 — re-confirmed **still open**: `loadInventory`'s success handler 3962–3986 and `drainPendingForeignReload_` 3046 both repaint only the inventory list via `renderInventory`, never `renderGoldSheetBody`/`renderDeleriumSheetBody`; the delerium counters are additionally rebuilt only by `refreshDeleriumStateFromInventory_`, which the reload never calls, so a foreign stock change leaves `deleriumOriginalQtys`/`deleriumSellQtys` stale as a sharper instance of the same root cause); run-68's **`sellDelerium` persists its optimistic `_opt_` negative rows + synthetic `_pending` "SELL … (saving…)" ledger entry to localStorage** RISK (FINDINGS:204 — re-confirmed **still open**: `sellDelerium` still calls `cacheInventoryRows(inventoryRows, inventoryResourceLedger)` at 5276 with no `_inFlightWrites++/--` bracket, unlike `receiveDelerium`'s guarded pre-flight cache 5170–5173); run-60/72's **`visibilitychange` sync-poll-stays-dead-if-backgrounded-within-first-20 s** RISK (FINDINGS:436, still open — resume guard `syncState.inventory.ts !== '0' || syncPollTimer` at 9008 unchanged); the run-64 `undoResourcePay` single-decrement (present, 6786), the run-55 `goldSheetMutated`-set-optimistically fixes (present at 5470/6147/6236), and the delerium "Gold received field only logged by Sell" guard (present, 5120). **Verified NOT a bug** (each checked because it looked like a stuck/lost/incorrect state): (a) **double-tap on Received/Sell/Split/Got-Paid/Pay** is naturally guarded without an explicit in-flight flag — `receiveDelerium`/`sellDelerium` call `refreshDeleriumStateFromInventory_` synchronously in the optimistic phase (5163/5265), resetting the counters so the second tap computes an empty `items` and bails on the "increment/select at least one" guard; and `receivedGold`/`splitGold`/`confirmPayWithReason` clear `goldSheetAmount` synchronously (5466/6232/6143) so the second tap reads `Number('')===0` and bails on the amount guard. (b) `giveItemToCharacter`'s captured `idx` (6054) can't drift during its own round-trip — `gasCall` holds `_inFlightWrites>0`, so any foreign `pollSync` defers via `pendingForeignReload` and `inventoryRows` is not replaced until after the give's handler runs; the single-row-of-a-rollup give limitation is the documented README TODO. (c) `confirmSellBatch`/`confirmSellItem` cache the optimistic (sold-down) rows before `gasCall` (6006/5651) like `sellDelerium` does, but — unlike `sellDelerium` — they add **no** synthetic `_pending` ledger entry, so the only cached artifact is reduced rows that the next `apiGetInventory` wholesale-replaces; self-heals cleanly, materially milder than the FINDINGS:204 case and not separately reported. (d) the delerium sheet's duplicate element IDs (`deleriumSheetNote`/`deleriumSheetStatus`/`deleriumReceivedBtn`) are mutually-exclusive treasurer-vs-non-treasurer / edit-vs-normal render branches, re-confirmed never simultaneously in the DOM (per run 63). **One new finding.**

#### BUG · Index.html:5186 · `receiveDelerium`'s server-error (`!res.ok`) branch reverts `inventoryRows` but never calls `renderInventory()`, so the inventory dashboard keeps showing crystals that were never received — the exact run-62 `receivedGold` defect (FINDINGS:417, fixed for gold), left un-fixed on the delerium receive path

**Story: Receive crystals — failure at the server call (`{ok:false}`) after the user has tapped Done and returned to the inventory list; state-machine on the optimistic-row revert transition.** `receiveDelerium` (5112) optimistically prepends one positive-`Qty` `_opt_rec_<size>` row per incremented size to `inventoryRows` (5140–5149) so the inventory dashboard's "Purple Rocks" crystal breakout reflects the receipt immediately, and sets `deleriumSheetMutated = true` (5159). Its `apiReceiveResource` success handler has three exits, and only one of them omits the inventory repaint:

```
5179  if (!res || !res.ok) {
5180    inventoryRows = previousRows;
5181    inventoryResourceLedger = previousLedger;
5182    refreshDeleriumStateFromInventory_();
5183    if (noteInput) noteInput.value = savedRecNote;
5184    if (status) status.innerHTML = `<span class="error">…Failed.…</span>`;
5185    cacheInventoryRows(inventoryRows, inventoryResourceLedger);
5186    renderDeleriumSheetBody();   // ← re-renders the (now-closed) sheet only
5187    return;                      // ← no renderInventory()
5188  }
…
5197    renderInventory();           // success path repaints inventory
…
5209    renderInventory();           // throw/transport-failure handler repaints inventory
```

The **success (`res.ok`) path** (5197) and the **transport-failure handler** (5209) both call `renderInventory()`; the **server-`{ok:false}` branch** (5186) is the lone path that reverts `inventoryRows` and repaints only `deleriumSheetBody`. `sellDelerium`'s matching `!res.ok` branch *does* call `renderInventory()` (5299), and run 62 already fixed this identical omission in `receivedGold` (FINDINGS:417, now present at 5491) — so `receiveDelerium` is the last un-fixed instance of the pattern.

Execution-trace of the failure: (1) User opens the Delerium sheet, increments (say) 2 chips + 1 shard, taps **Received** → three… (two) optimistic `+qty` rows prepended; `deleriumSheetMutated = true`; sheet shows "Saving…". (2) User taps **Done** → `closeDeleriumSheet` (4935) sees `deleriumSheetMutated` and calls `renderInventory()` (4940), so the inventory dashboard now paints the phantom crystals into the delerium breakout / "Purple Rocks" count. (3) User is looking at the inventory tab. (4) The server returns `{ok:false}` — a `LockService` timeout, a server-side validation guard the client's `items.length` check doesn't cover, or a quota blip. (5) The `!res.ok` branch reverts `inventoryRows = previousRows` (in-memory state now correct) and re-caches the corrected rows (5185), but re-renders only the hidden `deleriumSheetBody` and `return`s — the inventory list is **never repainted**, so the dashboard **keeps displaying the crystals that were never received**, and the "Failed." message lands on the closed sheet's `#deleriumSheetStatus` where the user can't see it.

State: stale until an unrelated repaint (a tab switch that re-runs `renderInventory`, the user's next write, or a foreign sync signature change) happens to fire. No persisted corruption — the phantom rows are never cached (the optimistic path leaves them out; the `!res.ok` branch caches the *reverted* rows), so a reload shows the truth — but the on-screen crystal total is silently wrong with the only explanation hidden on a dismissed sheet. This is strictly the run-62 `receivedGold` bug (FINDINGS:417), and it can be worse here because `receiveDelerium` prepends one optimistic row per size, so the inflation can span several crystal sizes at once. Fix: add `renderInventory();` to the `!res.ok` branch (before the `return` at 5187), matching `sellDelerium` (5299), `receiveDelerium`'s own throw handler (5209), and the run-62 `receivedGold` fix (5491).

### 2026-07-21 (run 73) — Sections audited: 9 (Index.html 3001–4500: the "inventory render, search, notes UI" slice. The physical range holds the sync/write plumbing `gasCall`/`drainPendingForeignReload_` 3035/3046, the localStorage inventory cache `bustInventoryCache`/`getCachedInventoryRows`/`getCachedInventoryPayload`/`cacheInventoryRows`/`primeInventoryCacheAfterAdd` 3152–3256, `populateCharacterSelectors`/`loadCharacters`/`loadQuickAddItems` 3195/3215/3207, the swipe-gesture state machine `initInventoryGestures` 3364, nav `setCommandMode` 3515, feedback `showSnackbar`/`setMainStatus`/`setNotesStatus` 3583/3598/3607, inventory search `handleCommandSearchInput`/`clearCommandSearch`/`handleSearchInput` 3622/3645/3888, the **combine** flow `findDuplicateInventoryCandidate`/`showCombineChoice`/`keepDuplicateInventoryItem`/`confirmCombineInventoryItem` 3696/3705/3723/3729, the inventory load+render core `inventorySignature_`/`loadInventory`/`setInventoryScope`/`updateInventoryScopeRow`/`renderInventory`/`renderInventoryRowCard`/`renderInventoryDashboard` 3911/3917/3996/4010/4043/4103/4146, the collaborative-sync poll `pollSync`/`startSyncPoll`/`stopSyncPoll` 4179/4210/4215, and the full **Party Notes** UI layer `loadNotes`/`noteCardHtml_`/`renderNotesList`/`toggleNotesArchived`/`handleNotesSearch`/`applyNotesFilters`/`toggleNotesPinFilter`/`openNoteForm`/`saveNoteForm`/`setNoteArchived_`/`archiveNote`/`unarchiveNote`/`toggleNotePin` 4251/4277/4306/4359/4364/4381/4386/4392/4465/4587/4615/4621/4625. Read out to the group builder in section 11 — `buildInventoryGroups`/`isDashboardResourceRow_`/`getInventoryGroupKey`/`renderInventoryGroup`/`rollupInventoryRows` 6554/6574/6583/6638/6662 — and the card-open entry points `handleInventoryCardClickById`/`openInventoryPrimaryActionById`/`openInventoryPrimaryAction` 6898/6903/6911.)

> **Stories traced** (happy → failure-at-step → navigate-away → friction, execution-trace + state-machine on every write path and UI component): **Inventory filter/search** (`handleCommandSearchInput` inventory branch 3625 → debounced `renderInventory` 3628 → scope filter 4058 → text filter 4066 → empty-guard 4073 → dashboard vs group render 4086); **View item details / tap card** (swipe-vs-tap dedupe: `pointerup` 3393 + `touchend` tap-branch 3477 share `lastTapOpenedAt`, and inline `onclick=handleInventoryCardClickById` 4133 is suppressed by `suppressInventoryClickUntil` 6899/6907 that `openInventoryPrimaryActionById` sets — re-verified the desktop pointerup+click double-fire is de-duped, and touch double-open across pointer/touch is de-duped by the 500 ms `lastTapOpenedAt` gate); **Combine duplicate** (`showCombineChoice` 3705 → `confirmCombineInventoryItem` 3729 optimistic filter-out-source/merge-target 3745 → `apiCombineInventoryItems`, double-tap guarded by the `pendingCombineChoice=null` seize 3732, failure restores `pendingCombineChoice`); **Create/Edit/Pin/Archive/Review-archived notes** (`saveNoteForm` 4465, `toggleNotePin` 4625, `setNoteArchived_` 4587, `renderNotesList` filter/sort 4306); plus the cross-cutting **Collaborative sync interference** / **Tab-switch in-flight** / **iOS background-foreground** stories through `pollSync` 4179, `_inFlightWrites`/`_inFlightNoteWrites`, `drainPendingForeignReload_`, and `loadInventory`'s `_inFlightWrites>0` re-defer 3972. Per protocol I re-verified prior in-range findings and did **not** re-report them: run-65's **`loadNotes` lacks the `_inFlightNoteWrites>0` re-defer guard** RISK (FINDINGS:128 — re-confirmed **still open**: `loadNotes` success still does unconditional `notesData = res.notes || []` at 4265 with no in-flight check) and its twin **`pendingForeignNoteReload` has no settle-time drain** RISK (FINDINGS:160 — re-confirmed **still open**: the four note-write settle handlers `saveNoteForm`/`setNoteArchived_`/`toggleNotePin` still `_inFlightNoteWrites--` inline with no `drainPendingForeignNoteReload_`; grep confirms `pendingForeignNoteReload` is still written only at 4185/4189 inside `pollSync`); the run-40/older **read-side foreign-notes-miss during in-flight `loadNotes`** RISK (FINDINGS:395, still open); run-60's **`visibilitychange` sync-poll-stays-dead-if-backgrounded-within-first-20 s** RISK (FINDINGS:436, still open); run-71's desktop treasurer sell-dock nav-clearance BUG and run-72's `showIdentitySheet` empty/DM-only-roster BUG (both still open). **Verified NOT a bug** (each checked because it looked like a stuck/lost state): (a) the **`toggleNotePin` failure-rollback uses the captured `n`** (4639/4647) rather than re-finding by ID like `setNoteArchived_` does (4596) — the in-code comment at 4584–4586 even claims parity ("matching toggleNotePin") that does not exist — but the only `notesData` *reassignment* that swaps in fresh server objects is `loadNotes` (4265); the other reassignment (`restoreNoteForm_` `.filter()` 4530) preserves element references, so the captured `n` stays live. A `loadNotes` reassignment during an in-flight pin is exactly the already-logged FINDINGS:128 clobber, whose fix (the in-flight guard) also closes this; not a separate bug. (b) `cacheInventoryRows`' `if (_inFlightWrites>0) return` (3233) means `primeInventoryCacheAfterAdd`'s cache write no-ops during an add, but the add's success handler re-caches after `gasCall` decrements the counter — no persisted-cache loss. (c) combine navigate-away is structurally unreachable (the `#combineSheet` full-screen backdrop covers the bottom nav, per run-71's z-index re-confirmation). **One new finding.**

#### BUG · Index.html:4066–4099 · Inventory filter/search silently drops every gold/delerium row: the filter path routes matches through `buildInventoryGroups`, which excludes "dashboard resource" rows (`isDashboardResourceRow_` 6566), but the empty-state guard runs on the *pre-group* `rows`, so a filter whose matches are all gold/delerium passes the guard and then paints a blank list under a misleading item count — the user searching for a resource row (or a note attached to one) is told nothing matched though matches exist. Also breaks the documented "filtered view shows notes inline for gold and delerium rows" behavior (README:44)

**Story: Inventory filter/search — happy path where the query matches only resource rows; state-machine on `renderInventory`'s empty-vs-count-vs-render transitions.** `renderInventory` (4043) applies the scope filter (4058) and then the text filter (4066) to produce `rows`. The text filter joins `[Item, Category, Rarity, Holder, Faction Relevance, Notes, Date Added]` (4068), so a gold row (`Item` matching `\b(gold|gp)\b`) or a delerium crystal row (`isDeleriumCrystalRow_`) **does** match a query on its name, holder, or note text, and lands in `rows`. The empty-state guard is checked next, on that pre-group set:

```
if (!rows.length) {                       // 4073
  ... 'No matches.' / 'Party pool is empty.' / ...
  list.innerHTML = ''; return;
}
...
if (!filter) { list.innerHTML = renderInventoryDashboard(rows); return; }   // 4086 — unfiltered only

// Filter path (4091–4100):
const groups = buildInventoryGroups(rows);
...
const rollupCount = groups.reduce((s, g) => s + rollupInventoryRows(g.rows).length, 0);
status.textContent = `${rollupCount} item${rollupCount !== 1 ? 's' : ''}`;
list.innerHTML = groups.map(g => renderInventoryGroup(g)).join('');
```

`buildInventoryGroups` (6554) runs every row through `if (isDashboardResourceRow_(row)) return;` (6566), and `isDashboardResourceRow_` (6574) returns true for every delerium crystal row and every `\b(gold|gp)\b` name — so **all** gold/delerium rows are discarded before grouping, and `Object.values(groupMap).filter(g => g.rows.length)` (6571) returns `[]` when they were the only matches.

Execution-trace of the failure state: user types a query that matches only resource rows (e.g. `delerium`, `gold`, `gp`, a payer/holder name that only appears on ledger rows, or the free-text of a gold/delerium note). (1) `rows` is non-empty → the `!rows.length` guard at 4073 is **skipped**, so the honest "No matches." copy never fires. (2) `filter` is truthy → the dashboard branch (4086) is skipped too, so the aggregate Gold/Delerium stat cards that would normally show these resources are also gone. (3) `buildInventoryGroups(rows)` returns `[]` → `rollupCount === 0` → `status.textContent = '0 items'` and `list.innerHTML = ''`. The user sees **"0 items" over a blank list** for a query that genuinely matched data. This is strictly worse than the `!rows.length` path: that one says "No matches" (truthful when nothing matched); this one asserts a count of zero *after* silently throwing away real matches, so the same blank screen now means two different things and neither surfaces the rows that exist.

Mixed-match variant (softer but same root cause): a query matching some ordinary items **and** some resource rows renders the ordinary items but silently omits the resource rows, and `rollupCount` under-reports the true match total — the header count disagrees with reality with no indication anything was dropped.

This is not merely a stale-doc mismatch: README:44 ("Filtered view shows notes inline for gold and delerium rows (single line, meta suppressed when notes present)") documents the **intended** behavior that a filtered view surfaces gold/delerium rows so their attached ledger notes are findable — a real use case (a player searching for the note on a specific Got-Paid or delerium sale). The current filter path implements the opposite: those rows are unreachable through search in either the filtered or the unfiltered view (unfiltered shows only the aggregate stat cards via `renderInventoryDashboard`, 4087/4146; `renderInventoryRowCard` 4103 was additionally trimmed to holder-only meta, 4116–4118, so even if a resource row reached the card renderer its note would not show inline). Net effect: gold/delerium ledger rows can never be located by text search, and attempting to do so returns a misleading "0 items"/blank result.

Fix options: (a) compute the empty-state after grouping — if `filter` is set and grouping yields zero display rows, render the honest "No matches." copy instead of "0 items" over a blank list, so search feedback is at least truthful; and/or (b) to actually restore the documented behavior, give the filter path a dedicated resource-row render (a lightweight single-line card that shows the row's note inline, per README:44) appended to the grouped output when `isDashboardResourceRow_` rows survive the text filter, so a player can find a gold/delerium note by searching for it. Option (a) removes the misleading feedback with minimal risk; (b) is the feature the README promises.

### 2026-07-20 (run 72) — Sections audited: 8 (Index.html 1501–3000: the "CSS continued, early JS, identity/init" slice. The physical range is the tail of the `<style>` block (identity-card/splash/mobile-sheet/bottom-nav/snackbar/notes-card CSS 1501–2315) + the body/header/identity-sheet HTML (2318–2931) + the top-level `<script>` state block and boot helpers (2995–3354). Per the cursor's authoritative *theme* text ("identity/init"), and because runs 60/66 already exhausted the identity happy/failure/cached traces and the `visibilitychange` poll-resume RISK, this run re-drove the **First open / identity picker** state machine end-to-end against current code and read out to every init hop: boot tail `updatePhoneClass` 3301 / `updateSellDockVisibility` 3341 / `setCommandMode` 3515 / `loadCharacters` 3215 / `loadQuickAddItems` 3207 / `loadMyIdentity` 4727 / `loadEquipmentIndex`; identity core `readCachedIdentity`/`cacheIdentity` 4660/4673, `applyIdentity` 4687, `loadFallbackCharacterIdentity` 4734, `showIdentitySheet` 4756, `initIdentityPicker` 4783, `confirmIdentity` 4795; the server side it calls — `apiGetMyCharacter` 932, `apiSetMyCharacter` 961, `saveUserProfile_` 1433, `apiGetCharacters` 864 (which splits active roster into non-DM `rows` 890 vs DM `dmRows` 899), `resolveIdentityForCharacter_` 1384; and the sync/init plumbing `gasCall`/`drainPendingForeignReload_` 3035/3046, `pollSync`/`startSyncPoll`/`stopSyncPoll` 4179/4210/4215, `visibilitychange` 9005, `loadInventory` 3917, `loadNotes` 4251.)

> **Stories traced** (happy → failure-at-step → navigate-away → friction, execution-trace + state-machine on every identity/init write path): **First open / identity picker** (boot `loadCharacters` + `loadMyIdentity` in parallel 9022/9024 → `apiGetMyCharacter` null → `showIdentitySheet` 4756 → tap card → `initIdentityPicker` delegated click 4787 → `confirmIdentity` 4795 optimistic `applyIdentity` + localStorage write + async `apiSetMyCharacter` re-apply 4818); **Returning user / cached identity** (`readCachedIdentity` 4660 ≤24 h → `applyIdentity(cached)` 4729 then `loadFallbackCharacterIdentity` server re-apply, scope-preservation `validStoredScope` 4701); plus the cross-cutting **Collaborative sync interference** / **Tab-switch in-flight** / **iOS background-foreground** stories through `gasCall`/`_inFlightWrites`/`drainPendingForeignReload_`, `pollSync` deferral 4184–4192, and the `visibilitychange` resume guard 9008. Per protocol I re-verified prior in-range/identity findings and did **not** re-report them: run-60's **`visibilitychange` sync-poll-stays-dead-if-backgrounded-within-first-20 s** RISK (FINDINGS:406/288 — re-confirmed present: the resume guard `syncState.inventory.ts !== '0' || syncPollTimer` still keys off "a poll cycle completed" not "polling should be active", and since the preceding `hidden` event always nulls `syncPollTimer`, the `|| syncPollTimer` disjunct is dead at every foreground — **still open, unchanged**); run-58/61's **`apiGetMyCharacter`-failure-bounces-cached-identity** RISK (FINDINGS:389 — re-confirmed **FIXED**: `loadFallbackCharacterIdentity` failure handler now `if (myCharacterName) return;` before wiping, present 4745); run-66's **`apiGetMyCharacter` trusts client hint without active-character check** RISK (FINDINGS:290, still open); the run-60 `.mobile-sheet` desktop display allow-list and `app-modal-open` scroll-lock (`syncModalOpenState`-reconciled) both re-confirmed benign; run-71's desktop treasurer sell-dock nav-clearance BUG (FINDINGS top of run 71, still open). **Verified NOT a bug** (each checked because it looked like a stuck/incorrect state): (a) the **double `applyIdentity`** on the cached path (once from `readCachedIdentity`, once from the server re-apply) is idempotent — `startSyncPoll` guards on `syncPollTimer` 4211, `loadNotes` guards on `notesLoading` 4253, and `cacheIdentity`/`populateCharacterSelectors` are pure re-writes; (b) `confirmIdentity`'s optimistic `isTreasurer: /^DM(\s|$)/i.test(character)` under-reports treasurer status for a **non-DM** treasurer for exactly one round-trip, but `apiSetMyCharacter` → `saveUserProfile_` → `resolveIdentityForCharacter_` returns the authoritative `isTreasurer` (admin-email OR DM) and the success handler re-applies it (964/1388/4818) — transient, self-correcting, not a bug; (c) a failed `apiSetMyCharacter` leaves the user correctly logged in via the optimistic apply + localStorage hint, and the next open re-resolves via `apiGetMyCharacter`'s client-hint fallback — the acknowledged comment at 4822 is accurate. **One new finding** — it is the still-open *other half* of run-65's `first-open picker stuck` BUG (FINDINGS:930), whose reconciliation marked it `✅ FIXED` after only the **failure** sub-case was addressed.

#### BUG · Index.html:4760–4762 · `showIdentitySheet` still hangs on "Loading characters…" when `apiGetCharacters` succeeds with **zero non-DM active characters** — the empty-response half of BUG:930 (marked ✅ FIXED in the run-65 reconciliation) was never fixed, and a DM-only active roster additionally strands the DM because the DM button is nested inside the `chars.length > 0` branch

**Story: First open / identity picker — failure-at-step (successful-but-empty roster) and the DM-only-roster variant; state-machine on the picker's idle→loaded→terminal states.** BUG:930 (run-56) logged two sub-cases of the identity picker stranding a first-time user on the `"Loading characters…"` placeholder: **(1)** `apiGetCharacters` *failure*, and **(2)** `apiGetCharacters` *succeeds but returns `rows: []`* (CHARACTERS sheet missing, all-inactive, or header mismatch). The run-65 reconciliation (FINDINGS:275, "first-open picker stuck (`loadCharacters` inline error+retry)") marked the whole finding `✅ FIXED` — but the fix it describes only covers sub-case (1): `loadCharacters`' failure handler now renders an inline error + Retry button (present, 3222–3228). Sub-case (2) is still live in current code:

```
function showIdentitySheet() {                        // 4756
  ...
  const chars = (characterOptions || []).filter(c => c.character);   // 4760
  if (!chars.length) {
    body.innerHTML = '<div class="status centered">Loading characters…</div>';   // 4762 — terminal, misleading
  } else {
    const dmChoice = (dmCharacterOptions && dmCharacterOptions[0]) || { character: 'DM Josh', player: 'Josh' };
    const playerGrid = ...chars.map(...);
    const dmButton   = `<div class="identity-dm-zone"><button ... data-character="${dmChoice.character}">DM</button></div>`;
    body.innerHTML = playerGrid + dmButton;            // 4777 — DM button ONLY rendered here
  }
  ...
}
```

Execution-trace: first-open, no cache → `apiGetMyCharacter` returns `character:null` → `showIdentitySheet()` (4738). If `apiGetCharacters` has already resolved with `rows:[]`, `characterOptions` is `[]`, so `chars.length === 0` → the body renders `"Loading characters…"` and the function returns. When `loadCharacters`' *success* handler re-invokes `showIdentitySheet()` (3221, the intended self-heal), it re-evaluates the same empty `chars` and re-paints the **identical** placeholder — a completed, successful load that is visually indistinguishable from an in-progress hang. There is no card, no error, no Retry, and (unlike sub-case 1) `loadCharacters` took the *success* branch so its retry affordance never fires. The user cannot select a character and cannot enter the app; only a manual reload escapes, and it lands on the same dead placeholder.

**New variant not in the original finding — DM-only active roster.** `apiGetCharacters` splits the active roster into non-DM `rows` (891) and DM `dmRows` (899); the client stores them as `characterOptions` / `dmCharacterOptions` (3218–3219). `chars` is derived only from `characterOptions` (the non-DM list). So if every *active* character is DM-prefixed (all players marked `Active? = No`, or an early-campaign / test roster that only has the DM row) — `rows` is `[]` but `dmRows` is non-empty — `chars.length === 0` takes the placeholder branch and returns **before** the DM button is built. The DM button lives exclusively inside the `else` (4770–4776), so the DM is stranded on "Loading characters…" even though a valid DM identity (`dmCharacterOptions[0]`) is sitting in memory ready to select. The picker's ability to seat the DM is incorrectly coupled to the presence of ≥1 non-DM player.

Impact is confined to a degenerate roster state (zero active non-DM characters), so this is BUG-minor, not a mainline blocker — but it is a genuine stuck state with actively misleading feedback ("Loading…" when loading is done), it blocks *every* downstream story for the affected user, and it is the un-fixed remainder of a finding the ledger currently records as closed. Fix (mirrors the original BUG:930 remedy that was only half-applied): give `showIdentitySheet` a real terminal state when `chars.length === 0` — render a "No characters found — Retry" affordance **and** still emit the DM button from `dmCharacterOptions` so a DM-only roster can seat the DM; keep the "Loading characters…" copy only for the genuine pre-load state (e.g. gate it on `!characterOptions.length && loadCharactersInFlight` vs. a loaded-but-empty result).

### 2026-07-19 (run 71) — Sections audited: 7 (Index.html 1–1500: the "HTML structure, CSS" slice. The `<style>` block runs to 2315 and the body/sheet HTML from 2318, so per the cursor's authoritative *theme* text I audited the CSS design-token/reset/component layer 1–1500 **plus** the shared structural layer every story routes through: the full `.mobile-sheet` z-index ladder (base 70; `#sellItemSheet`/`#giveItemSheet`/`#payReasonSheet` 80, `#descActionSheet` 81, `#sellBatchSheet` 82, `#identitySheet` 90 — 1424/231/2845/2869/2881/2907/2923), `body.app-modal-open` scroll-lock (110), the `.snackbar` transient-feedback surface (1665, z-index 60), the `.bottom-nav` (1655, z-index 30) and its treasurer sell-dock row (`.bottom-nav .sell-batch-btn` 844), and the `main`/body bottom-padding chain (385/1737/1788/1811) that clears the fixed nav. Read out to the driving JS as needed: `setCommandMode` 3515, `syncModalOpenState` 3683, `showSnackbar`/`setMainStatus` 3583/3598, `updateSellDockVisibility` 3341, `updatePhoneClass` 3311, and the sheet open/close + confirm handlers for every story below.)

> **Stories traced** (happy → failure-at-step → navigate-away → friction, execution-trace + state-machine on each write path, all through the shared sheet/z-index/scroll-lock/feedback structure defined here): **View item details / Give item to character / Sell item / Remove item** (description sheet `#descriptionSheet` 70 → `openDescActionSheet` 7162 sub-sheet `#descActionSheet` 81 → `_confirmDescActionGive_`/`_confirmDescActionSell_`/`_confirmDescActionRemove_`/`_confirmDescActionAdd_` 7279/7481/7547/7380); **Add library/custom item** (`addInventoryItem` 8806, `_confirmDescActionAdd_` 7380); **Combine duplicate** (`showCombineChoice` 3705 / `confirmCombineInventoryItem` 3729 → `#combineSheet` 70); **Edit inventory item** (`#inventorySheet` 70 → `saveInventoryEdits` 7987, status routed to in-sheet `#sheetEditStatusMessage` via `getEditStatusElement` 7973); **Swipe delete** (`handleInventoryDeleteActionById` 8061, two-tap arm); plus the cross-cutting **Tab-switch in-flight** / **Collaborative sync interference** / **iOS background-foreground** stories through `setCommandMode` 3515, `gasCall`/`_inFlightWrites`/`drainPendingForeignReload_` 3035–3051. Per protocol I re-verified prior in-range findings and did **not** re-report them: run-59's **`app-modal-open` scroll-lock discipline is clean** — `syncModalOpenState` 3683 recomputes the class from the live `.mobile-sheet.active` set on every open/close, so a stacked descActionSheet-over-descriptionSheet keeps the lock until the last sheet closes (re-confirmed present); run-59's **z-index ladder is collision-free and the bottom-nav (30) sits beneath every sheet (70–90)** so "switch tabs while a sheet is open" stays **structurally unreachable** (re-confirmed); run-63's **`--keyboard-offset` is consumed by `.mobile-sheet-actions` (1604) but the `html.is-phone .mobile-sheet-actions` override (1973) drops it and the var is still only bound for the notes sheet** — the iOS-keyboard-over-the-action-bar RISK is **still open** on the primary (phone) platform; run-63's **`quickEditInFlight` never-resets-on-dropped-callback** RISK — **still open**. **Verified NOT a bug** (each checked because it looked like feedback loss): the app's single transient surface `showSnackbar` is z-index 60 (**below** every sheet), so I traced every `setMainStatus` call that fires from a sheet-hosted flow — `_confirmDescActionGive_`/`Sell_`/`Remove_`/`Add_` (7341–7347/7502–7504/7586–7587/7438–7439) and `confirmCombineInventoryItem` (3751) all call `closeDescActionSheet`/`closeDescriptionSheet`/`keepDuplicateInventoryItem` **before or synchronously around** the snackbar, so the sheet is gone by paint and the snackbar is visible; in-sheet failures (edit-save 8007, combine 3742) route to the sheet's own status div, not the snackbar — no hidden feedback found. Also confirmed the four duplicate IDs (`dTotalLabel`, `deleriumReceivedBtn`, `deleriumSheetNote`, `deleriumSheetStatus`, 4885–4989) are all mutually-exclusive ternary/treasurer-vs-non-treasurer branches of the dynamically-rendered delerium sheet (section 10) — never simultaneously in the DOM — so `getElementById` never collides. **One new finding.**

#### BUG · Index.html:3346 · Treasurer sell-dock grows the bottom nav to ~148px but only the phone code path grows the nav-clearance variable — on desktop `--nav-height` stays 86px, so the bottom ~34px of the inventory list (and the last card) sits clipped behind the fixed nav, unreachable by scrolling

**Story: Sell item batch / browse inventory as treasurer on desktop; state-machine on the nav-height ↔ content-padding coupling.** The "Sell Items" dock is a full-width row that lives **inside** the fixed bottom nav, shown only for the treasurer on the inventory tab (`#sellDockBtn` 2986; `.bottom-nav .sell-batch-btn { grid-column: 1 / -1 }` 844). Adding that row makes the nav taller than the plain two-button nav. The scrollable content clears the fixed nav via `main`'s bottom padding, which is derived from `--nav-height`:

- base/phone `main { padding-bottom: calc(var(--nav-height) + safe-bottom + 28px) }` (385), desktop the same with `--nav-height: 86px` (1737/1724).
- On phone the body/main padding instead uses `--phone-nav-height` (1788/1811).

`updateSellDockVisibility` (3341) is the one place that reconciles nav height with the sell dock, and it does so **only on the phone path**:

```
if (isMobileLayout()) {
  const scale = parseFloat(...'--phone-scale') || 2.5;
  document.documentElement.style.setProperty('--phone-nav-height', `${Math.round((show ? 148 : 84) * scale)}px`);
}
```

So when the dock appears on phone, `--phone-nav-height` grows from 84→148 units and the body/main padding follows — content stays clear. But there is **no `else` branch**: on desktop (a real ≥700px browser with no `is-phone`/`pointer:coarse` — e.g. the treasurer opening the web app on a laptop), `--nav-height` is never touched. It stays **86px**, so `main`'s bottom padding is `86 + 28 = 114px`, while the nav-with-dock actually measures ~**148px** (10 top pad + 62px `.sell-batch-btn` row + 10 gap + 56px `--nav-button-height` tab row + 10 bottom pad = 148 — the very number the phone branch hard-codes). The fixed nav therefore overlaps the bottom `148 − 114 = ~34px` of scrollable content. When scrolled fully to the bottom, the last inventory row's lower ~34px is covered by the sell dock and cannot be revealed by scrolling further (the document has no more travel). The plain (non-treasurer, or non-inventory-tab) desktop nav is ~76px < 114px padding, so this clipping appears **only** in the treasurer + inventory-tab + desktop-layout state — exactly the state that also renders the extra row.

That the phone branch already encodes `148` for this same dock shows the correction was known but not mirrored to the desktop custom property. Fix: in the non-mobile branch of `updateSellDockVisibility`, set `--nav-height` to `show ? 148 : 86` (px) so the desktop `main`/body padding tracks the dock the same way the phone path already does; or drive both platforms off one measured `bottomNav.offsetHeight` written into the clearance variable. Impact is confined to layout (obscured content, no data loss), desktop-treasurer only — hence BUG-minor, not RISK.

### 2026-07-18 (run 70) — Sections audited: 6 (Code.js 3501–end: the "batch sell, give, remove, quick-adjust, sync, utilities" slice. The physical 3501–4060 tail holds `apiDeleteInventory` 3436, `apiUndoMemberSend` 3509, `apiCombineInventoryItems` 3547, `apiGetCurrencyQuickEdit` 3670, `apiAdjustCurrency` 3712, `apiAdjustInventory` 3722, `apiSetItemQuantity` 3845, and the utility helpers `normalizeForClient_` 3956, `ensureInventoryHeaders_` 3971, `fillMissingInventoryRarity_` 3988, `findInventoryRowById_` 4048. Per the cursor's authoritative *theme* text — and because runs 58/62/63/65 already exhausted the quick-adjust write path and its error-mapping — this run centered on the **quick-adjust** state machine end-to-end and the two theme functions no prior run has given a dedicated trace: `apiUndoMemberSend` (the "give"/member-send reversal) and the batch `apiSellInventoryBatch` 664 / `apiGiveInventoryBatch` 767 merge tail 822–842. Read out to the client callers `openInventoryPrimaryAction` 6911, `getQuickEditType` 7629, `openQuickEditPanel` 7656, `populateQuickSize` 7728, `onQuickSizeChange` 7743, `confirmQuickEdit` 7809, `closeQuickEditPanel` 7782, and the undo path `undoResourcePay` 6786.)

> **Stories traced** (happy → failure-at-step → navigate-away → friction, execution-trace + state-machine on every write path): **Quick-adjust currency/delerium** (`openInventoryPrimaryAction` 6911 → `getQuickEditType` 7629 → `openQuickEditPanel` 7656 optimistic-local-`selectedQuickEdit` + async `apiGetCurrencyQuickEdit` 3670 refresh with the `selectedQuickEdit.itemId !== row['Inventory ID']` staleness guard 7698 → `confirmQuickEdit` 7809 → `apiAdjustInventory` 3722 / `apiSetItemQuantity` 3845; size-reclassify confirm `onQuickSizeChange` 7743); **Undo last pay — member-send branch** (`undoResourcePay` 6786 creditId branch 6817 → `apiUndoMemberSend` 3509); **Undo last pay — purchase branch** (`apiDeleteInventory` 3436 `reverseLedgerEntry` → `deleteResourceLedgerRowsForInventory_` 2013); **Sell item / Remove item** (batch `apiSellInventoryBatch` 664, gold-credited-before-row-delete ordering re-confirmed present 717–753) and **Give item to character** (`apiGiveInventoryBatch` 767 multi-row drain-then-merge, `findMatchingInventoryRow_`/`mergeIntoExistingRow_` merge tail 822–842); **Combine duplicate** (`apiCombineInventoryItems` 3547 — delete-source-then-rewrite-target, re-confirmed against run-68 trace, not re-reported); plus the cross-cutting **Collaborative sync interference** / **Tab-switch in-flight** / **iOS background-foreground** stories through `gasCall`/`_inFlightWrites`/`drainPendingForeignReload_` (the quick-adjust path routes both writes through `gasCall`, so it inherits the in-flight foreign-reload defer the notes path lacks — run-69). Per protocol I re-verified prior in-range findings and did **not** re-report them: the **`quickEditInFlight` never-resets-on-a-dropped-callback** RISK (FINDINGS run-63 twin at :3481; `closeQuickEditPanel` 7782 still deliberately does not reset the flag, 7784–7786, and no `visibilitychange` reset exists — **still open**); the run-51/`#1391` **delerium size-merge** server RISK (`apiAdjustInventory` still renames the row keeping `oldQty+delta`, present 3757–3768 — **still open**); the run-65 **error-allowlist** BUG (`Cannot remove more than the N currently held.` at 3753 still maps to "Request failed." — **still open**); the run-51 **health-potions-never-quick-edited** IDEA (`getQuickEditType` 7629 still returns `''` for potions, so the catalog's "quick-adjust health potions" routes to the description sheet, not the quick sheet — **still open**); the run-58 Cancel-mid-flight double-apply BUG (FIXED, 7784–7786) and delta-0 phantom-write RISK (FIXED, `apiAdjustInventory` early-returns at 3750). **Verified NOT a bug** (checked because it looked like silent data loss): a delerium row whose name lacks a canonical size token (e.g. "Aqua Delerium") is **not** silently renamed by a quick-adjust — `openQuickEditPanel`/`apiGetCurrencyQuickEdit` set the size dropdown to the un-normalized name, which is not a valid `<option>`, so `select.value` resolves to `''`, `confirmQuickEdit` sends `size:''`, and the server's `if (size)` guard (3764) skips the rename. Also confirmed quick-adjust is fully **pessimistic** (no optimistic row mutation before the server confirms), so its failure/navigate-away paths need no rollback — the only in-flight defect is the already-logged `quickEditInFlight` latch, and the full-screen `.mobile-sheet` (`z-index:70; inset:0`, 1424) covers the bottom nav, so a tab-switch cannot orphan an open quick sheet. **One new finding.**

#### RISK · Code.js:3527–3538 · `apiUndoMemberSend` is not the all-or-nothing reversal its name and caller comment claim — it deletes whichever of the two rows it finds and returns `{ok:true}` unconditionally, so a half-present pair yields a half-reversal (gold created or destroyed) reported to the client as a clean undo, with the undo affordance already discarded so it can't be retried

**Story: Undo last pay (member-send branch) — the credit row is gone by the time the payer taps Undo; execution-trace + state-machine on the reversal.** A member-send pay creates two inventory rows: a **deduct** row (party-pool gold down) and a **credit** row (recipient's gold up), net-conserving. `undoResourcePay` (Index:6786) arms the undo with both IDs (`deductId` 6788, `creditId` 6792) and, on tap, **optimistically removes both rows locally** (6796–6798), nulls `lastResourceUndo[resource]` (6801) — permanently retiring the Undo button — and fires `apiUndoMemberSend({deductId, creditId})` (6819) with the inline comment *"Member-send path: delete both rows atomically."*

Server-side (3527–3538) there is no atomicity and no all-or-nothing check:
...
return { ok: true, message: 'Member send reversed.' };   // <-- unconditional, even if 0 or 1 rows matched
```

`getInventoryRowObjectById_` returns `null` for a missing ID without throwing (only a malformed ID trips `validateId_`), so when exactly one of the two rows is absent the function silently reverses just that half and still reports full success. Concrete gold-creation trace:

1. Player A pays member M 50 gp (member-send). Deduct row D (pool −50) + credit row C (M +50) are written; A's client arms `lastResourceUndo['gold'] = {item: D, creditItem: C}`.
2. Before A taps Undo, the party's gold is actively managed by someone else this session — M's credit row C is consumed by a **Sell for Gold**, a **Give**, a **Combine**, or a manual **Delete/Edit** (all of which delete or merge-away C's `Inventory ID`). Row C no longer exists under `creditId`. (This is the ordinary multiplayer case the app is built for — gold rows are not immutable between a pay and its undo.)
3. A taps **Undo**. Client removes D and C locally, nulls the undo state, calls `apiUndoMemberSend`.
4. Server: `deductFound` = D, `creditFound` = null. It deletes D and reverses D's ledger; C is untouched. Returns `{ok:true, message:'Member send reversed.'}`.
5. Client success handler (6820–6824) sees `res.ok`, keeps the optimistic state (both gone), re-caches. **Nothing signals the partial outcome.**
6. On the next sync/reload, C reappears from the server (it was never deleted). Final state: the pool deduction is reversed (**pool +50 restored**) **and** M still holds the 50 gp credit → **50 gp materialized out of nothing**, visible to every client. The mirror case (deduct row gone, credit present) **destroys** 50 gp the same way. Neither is recoverable through the UI — the Undo button is gone (6801) and there is no error to prompt a reload.

This is strictly worse than the sibling write endpoints, which all guard their preconditions: `apiCombineInventoryItems` returns `{ok:false, error:'Inventory item not found.'}` when either row is missing (3574–3576), and `apiDeleteInventory` returns the same when its single row is absent (3458–3460). `apiUndoMemberSend` should apply the same rule at the pair level: if `!deductFound || !creditFound`, delete nothing and return `{ok:false, error:'Original transaction rows no longer exist — reload and check the ledger.'}`, so the client's `rollback()` (6804–6815) restores the undo affordance and the ledger instead of baking in a half-reversal. (Secondarily, this endpoint is the only inventory writer in the file with **no `auditWrite_`** on either the success or failure path — a gold-affecting reversal that leaves no audit row — but the partial-reversal-reported-as-success is the behavioral defect.)

### 2026-07-17 (run 69) — Sections audited: 5 (Code.js 2801–3500: the "delerium, custom inventory, notes" slice. The line range has drifted so this slice now physically holds the tail of `apiDepleteResource` (2825), `apiUpdateLedgerNote` 2850, `apiReceiveResource` 2906 (delerium branch 2954), `apiSellInventoryItem` 2995, `apiSellDelerium` 3053, `apiSplitGold` 3138, `apiSendGoldToMember` 3253, `apiUpdateInventory` 3345, `apiDeleteInventory` 3436 — most of which run 68 already traced. Per the cursor's authoritative *theme* text ("delerium, custom inventory, notes"), and because run 68 exhausted the delerium/gold write endpoints, this run targeted the theme's untraced surface: the **Party Notes v2** endpoints `makeNoteId_` 2202 / `apiGetNotes` 2209 / `apiCreateNote` 2257 / `apiUpdateNote` 2298 / `apiArchiveNote` 2329, the **custom inventory** endpoint `apiAddCustomInventory` 2470, and the notes sync plumbing `bumpSync_`/`apiGetSyncState` 2146/2156 — read out to their untraced client callers `loadNotes` 4251, `renderNotesList` 4306, `noteCardHtml_` 4277, `openNoteForm` 4392, `saveNoteForm` 4465 (create + edit), `setNoteArchived_` 4587, `archiveNote`/`unarchiveNote` 4615/4621, `toggleNotePin` 4625, and the collaborative-sync poll `pollSync` 4179)

> **Stories traced** (happy → failure-at-step → navigate-away → friction, execution-trace + state-machine on every write path): **Create note** (`saveNoteForm` add-branch 4517: optimistic `NOTE_TEMP_<ts>` push 4522, `notesLoaded=true`, close+render, `_inFlightNoteWrites++` 4527 → `apiCreateNote` 2257 append + `bumpSync_('notes')` → success reconciles tempId→`res.note` 4549–4556, failure `restoreNoteForm_` 4529 stashes `pendingFailedNote` on navigate-away 4535); **Edit note** (`openNoteForm('edit')` 4392 with `_notesActionInFlight` re-open block 4393, `saveNoteForm` edit-branch 4483: optimistic `Object.assign` 4487 + `backup` capture 4486, `_inFlightNoteWrites++` + `_notesActionInFlight.add` 4493–4494 → `apiUpdateNote` 2298 field-allowlist patch, failure re-finds by id and restores `backup` 4501–4502); **Pin note** (`toggleNotePin` 4625 optimistic flip → `apiUpdateNote` `{Pinned}` 4651, failure revert); **Archive note / Unarchive / Review archived** (`archiveNote` confirm 4617 → `setNoteArchived_` 4587 optimistic `archived` flip moving the card into the collapsible `notes-archived-group` 4345–4353, `apiArchiveNote` 2329 default-true toggle, failure re-finds+reverts 4596, errors now on `#notesStatus` — run-65 BUG FINDINGS:277 re-verified FIXED, present 4603/4610); plus the cross-cutting **Collaborative sync interference** / **Tab-switch in-flight** / **iOS background-foreground** stories through `pollSync` notes branch 4194–4204, `_inFlightNoteWrites`/`pendingForeignNoteReload`, and `visibilitychange` immediate `pollSync()` 9010. **Custom inventory** (`apiAddCustomInventory` 2470) and the in-range delerium/gold endpoints were re-confirmed against run-68 traces and NOT re-reported (`apiSellDelerium` non-atomic N+1 append still open at FINDINGS:97; `apiSellInventoryItem` delete-before-credit present 3020; `apiSplitGold`/`apiSendGoldToMember` batched `setValues` present 3213/3306; `apiReceiveResource` delerium per-size append present 2971). Confirmed the client/server note category lists agree (`NOTE_CATEGORIES` Index:3122 == `PARTY_NOTES_CATEGORIES` Code:2179), `apiGetSyncState` always returns a `notes` field so `pollSync`'s `res.notes.ts` never NPEs (2163), and the create-note tempId/noteId reconciliation (4549–4556) prevents duplicate-or-lost rows even when a reload lands mid-flight. Two related-but-distinct prior notes-sync findings re-confirmed **still open** and referenced (not duplicated): the run-65 RISK (FINDINGS:267, `loadNotes` no-ops on its `notesLoading` read-reentrancy guard while `syncState.notes` is advanced — a *read*-side stale-view miss) and the run-40 defer-guard (`pendingForeignNoteReload` exists, present 4195). **Two new findings** — both are the *write*-side mirror of those read-side issues: the notes reload path lacks the write-clobber protection the inventory reload path has.

#### RISK · Index.html:4251,4265 · `loadNotes` lacks the `_inFlightNoteWrites > 0` re-defer guard that `loadInventory` has (3972), so a poll-triggered `loadNotes(true)` can fire while a note write is in flight and wholesale-replace `notesData` with a pre-commit server snapshot — silently discarding an optimistic pin/edit/archive that its success handler never re-applies

**Story: Collaborative sync interference / Pin note / Edit note — foreign notes write coincides with a local note write in flight; state-machine on the optimistic-vs-reload transition.** The inventory reload path defends this exact race in two places: `gasCall` drains a deferred foreign reload the instant the last write settles (`drainPendingForeignReload_()` 3038–3039/3046), **and** `loadInventory`'s success handler re-checks `if (_inFlightWrites > 0) { pendingForeignReload = true; markInventoryReady(); return; }` (3972) before it ever assigns `inventoryRows`. The notes path has **neither**. `loadNotes` (4251) guards only `if (notesLoading) return;` (4253) and, on success, unconditionally does `notesData = res.notes || []` (4265) — no `_inFlightNoteWrites` check anywhere.

That guard matters because `pollSync`'s notes branch *can* reach `loadNotes(true)` while `_inFlightNoteWrites > 0`. Re-read 4194–4204:

```
if (res.notes.ts !== syncState.notes.ts) {
  if (res.notes.by !== syncClientId && _inFlightNoteWrites > 0) {
    pendingForeignNoteReload = true;                 // defer — syncState.notes NOT advanced
  } else {
    syncState.notes = res.notes;
    if (res.notes.by !== syncClientId || pendingForeignNoteReload) {
      pendingForeignNoteReload = false;
      loadNotes(true);                               // <-- can run with _inFlightNoteWrites > 0
    }
  }
}
```

The in-flight defer only triggers when `res.notes.by !== syncClientId`. When a **local** note write has bumped `SYNC_NOTES` to `{by: syncClientId}`, the first condition is false regardless of `_inFlightNoteWrites`, so control falls to the `else` arm; there `loadNotes(true)` fires whenever `pendingForeignNoteReload` is already set — **even though a local write is still in flight**. And `pendingForeignNoteReload`, once set, is never cleared except inside this `else` arm (there is no settle-time drain analogous to `drainPendingForeignReload_`, see the second finding), so it lingers across writes.

Concrete stuck-state trace (Pin note is the clearest victim because its success handler is a no-op — it only acts on `!ok`):
1. Two players edit notes this session. A foreign note write lands while some local note write is in flight → `pollSync` sets `pendingForeignNoteReload = true` and does **not** advance `syncState.notes`. The flag lingers.
2. Local user taps **Pin** on note X: `toggleNotePin` (4625) sets `n.pinned = true` optimistically, renders, `_inFlightNoteWrites++` (4633), fires `apiUpdateNote({Pinned:true})`.
3. Before the pin's handler fires, the 20 s poll runs. A local write has made `res.notes.by === syncClientId`, so the defer is bypassed → `else` arm → `pendingForeignNoteReload` is true → `loadNotes(true)` executes **with `_inFlightNoteWrites === 1`**.
4. `loadNotes`' `apiGetNotes` read races ahead of the pin's `setValue` commit, so it returns X with `pinned=false`. Its success handler does `notesData = res.notes` (4265) — the optimistic pinned X is clobbered; the UI repaints X unpinned.
5. The pin's `apiUpdateNote` then commits server-side (X.pinned=true) and its success handler runs — but `res.ok` is true, so it does **nothing** (4635–4642 only revert on `!ok`). The UI keeps showing X unpinned.
6. `syncState.notes.ts` has already advanced to include the pin's bump, so no future poll sees a ts change. **X displays unpinned indefinitely though it is pinned on the server** — recoverable only by the user's next note write or a TTL-expired tab re-entry.

The same clobber hits the **edit** branch of `saveNoteForm` (its success handler also no-ops on `res.ok`, 4496–4505) and `setNoteArchived_` (4598–4604) — any optimistic mutation whose server confirmation carries no re-apply is silently lost if a reload replaces `notesData` between the optimistic paint and the server commit. **Create** is the one exception: its tempId→noteId reconciliation (4549–4556) re-adds the server row, so it survives. Fix: mirror `loadInventory` — in `loadNotes`' success handler (before line 4265) add `if (_inFlightNoteWrites > 0) { pendingForeignNoteReload = true; return; }`, so a reload that races an in-flight note write re-defers instead of overwriting optimistic state. (This is the write-side complement to the run-65 read-side miss at FINDINGS:267; both live in the `loadNotes`/`pollSync` coupling.)

#### RISK · Index.html:4527,4547,4594,4633 · `pendingForeignNoteReload` has no settle-time drain — unlike inventory's `pendingForeignReload` (drained by `drainPendingForeignReload_()` on every write settle), a deferred foreign note reload waits until the *next* 20 s poll, so a note another player added is invisible for up to a full poll interval after the local write that deferred it completes

**Story: Collaborative sync interference — foreign note write deferred behind a local note write; efficiency/latency + the enabling factor for the finding above.** Inventory writes go through `gasCall`, whose success/failure handlers call `drainPendingForeignReload_()` (3038–3039), so the moment `_inFlightWrites` hits 0 the deferred foreign reload fires immediately (3046–3049). Notes writes do **not** use `gasCall` — `saveNoteForm` (create 4527/4547/4570, edit 4493/4497/4508), `setNoteArchived_` (4594/4600/4608), and `toggleNotePin` (4633/4637/4646) each `_inFlightNoteWrites++/--` inline and **none** calls any drain. Grepping confirms `pendingForeignNoteReload` is written in exactly two places: set `true` at 4196 and cleared only at 4200, both inside `pollSync`.

Consequence: when a foreign note write is deferred because a local note write is in flight (4195–4196), the reload is not applied when that local write settles — it waits for the next `pollSync` tick (≤ `SYNC_POLL_MS` = 20 s later, 3120) or the next foreground `visibilitychange` `pollSync()` (9010). The foreign player's new/edited/pinned note simply doesn't appear for that window, even though the local user is now idle and the app could refresh instantly. It always self-heals (the deferral deliberately leaves `syncState.notes` un-advanced so the next poll re-detects), so this is latency/inconsistency, not data loss — but it is both (a) a UX regression versus the inventory tab's instant convergence and (b) the mechanism that keeps `pendingForeignNoteReload` lingering long enough to trigger the write-clobber in the finding above. Fix: give the four note-write settle handlers a `drainPendingForeignNoteReload_()` call (symmetric with `drainPendingForeignReload_`) that, when `_inFlightNoteWrites <= 0 && pendingForeignNoteReload`, clears the flag and calls `loadNotes(true)` — combined with the first finding's in-flight guard in `loadNotes`, this makes the notes sync path converge exactly like the inventory path.

### 2026-07-16 (run 68) — Sections audited: 4 (Code.js 2101–2800: the "sell, combine, gold ops" slice — the resource/gold write endpoints this range holds: `apiGetInventory` 2108, sync `bumpSync_`/`apiGetSyncState` 2146/2156, the Party-Notes v2 endpoints `apiGetNotes`/`apiCreateNote`/`apiUpdateNote`/`apiArchiveNote` 2209–2352, the inventory add endpoints `apiAddInventory`/`apiAddCustomInventory`/`apiQuickAddInventory` 2354/2470/2577, and the money/resource core every gold + delerium story routes through — `apiDepleteResource` 2718, `apiUpdateLedgerNote` 2850, `apiReceiveResource` 2906, `apiSellInventoryItem` 2995, `apiSellDelerium` 3053, `apiSplitGold` 3138, `apiSendGoldToMember` 3253 — read out to their client callers `receivedGold` 5434 / `confirmPayWithReason` 6099 / `splitGold` 6208 / `payResource` 6709 / `undoResourcePay` 6786 / `receiveDelerium` 5112 / `sellDelerium` 5225 / `updateLedgerNoteFromBottom` 6453 and the `apiCombineInventoryItems` 3547 endpoint the "combine" story names)

> **Stories traced** (happy → failure-at-step → navigate-away → friction, execution-trace + state-machine on every write path): **Receive gold — Got Paid** (`receivedGold` 5434 optimistic `_opt_gold_` row + `_pending` RECEIVE ledger → `apiReceiveResource` 2906 gold branch, single append, `receiveLedgerItem` from→to, `removePending` on settle); **Pay gold — Purchase/character** (`openPayReasonSheet` 5515 → `confirmPayWithReason` 6099 → `apiDepleteResource` 2718 [Purchase/personal] / `apiSendGoldToMember` 3253 [member-send, batched 2-row `setValues` 3306], optimistic deduct+credit rows, `lastResourceUndo['gold']` arming 6175); **Split gold evenly** (`splitGold` 6208 pending-ledger-only optimism → `apiSplitGold` 3138 batched N-row `setValues` 3213, `perMember=floor` + remainder-to-pool); **Undo last pay** (`undoGoldPay` 5428 / `undoResourcePay` 6786 → `apiUndoMemberSend` [creditId branch] / `apiDeleteInventory` reverseLedgerEntry [purchase branch], `rollback` restore); **Edit ledger note** both resources (`selectLedgerRowForEdit` 6405 amount-`readOnly` latch, `cancelLedgerEdit` 6436, `updateLedgerNoteFromBottom` 6453 → `apiUpdateLedgerNote` 2850 entryId-then-timestamp match); **Receive crystals** (`receiveDelerium` 5112, `_inFlightWrites++/--`-guarded pre-flight cache 5170) and **Sell crystals** (`sellDelerium` 5225 incl. 0-gp inline confirm → `apiSellDelerium` 3053); **Quick-add** currency/delerium/scroll (`apiQuickAddInventory` 2577, always-append, ledger for currency/delerium); plus the cross-cutting **Collaborative sync interference** / **Tab-switch in-flight** / **iOS background-foreground** stories through `gasCall`/`_inFlightWrites`/`drainPendingForeignReload_` and `cacheInventoryRows` in-flight guard 3232 / `loadInventory` cold-paint-then-replace 3937–3977. Per protocol I re-verified prior in-range fixes landed and did NOT re-report them: `receivedGold`'s `!res.ok` branch now calls `renderInventory()` (run-63 BUG, FINDINGS:221, present 5491); `apiSplitGold` single batched `setValues` (run-56 RISK, FINDINGS:438, present 3213); `apiSendGoldToMember` single batched `setValues` (run-56 RISK, FINDINGS:340, present 3306); `apiSellInventoryItem` delete-before-credit ordering (run-56 RISK, FINDINGS:353, present 3020); `undoGoldPay` repaints the ledger (FINDINGS:386, present 5430); the delerium "Gold received field only logged by Sell" guard (run-37 RISK, present 5120); the `undoResourcePay` single-decrement via gasCall (run-64 BUG, present); and the run-64 `receiveDelerium` `_inFlightWrites`-bracketed pre-flight cache (FINDINGS:1391, RESOLVED, present 5170–5173) — **whose symmetric fix is missing from `sellDelerium`, the first finding below.** **Two new findings.**

#### RISK · Index.html:5276 · `sellDelerium` persists its optimistic `_opt_` negative rows and the synthetic `_pending` "SELL … (saving…)" ledger entry to localStorage — the exact `receiveDelerium` defect fixed in run-64 (2026-07-02) was never mirrored onto the sell path

**Story: Sell crystals — navigate-away / iOS-background mid-flight; state-machine on the optimistic-cache transition.** `cacheInventoryRows` (3232) is a no-op only while `_inFlightWrites > 0` (`if (_inFlightWrites > 0) return;`). `receiveDelerium` relies on exactly that: it brackets its pre-flight cache with a temporary `_inFlightWrites++ … cacheInventoryRows … renderDeleriumSheetBody(); _inFlightWrites--` (5170–5173) — with the inline comment explaining the point is to keep "the optimistic rows and the synthetic `_pending` 'Saving…' ledger entry out of localStorage … so a navigate-away mid-flight then repaints the last confirmed state, not a phantom entry with no counter to clear it." This is the run-64 RISK marked RESOLVED at FINDINGS:1391/1413.

`sellDelerium` (5225) contains **no `_inFlightWrites` reference at all** (grepped: none in 5225–5283). Its pre-flight sequence is:

```
5264  deleriumSheetMutated = true;
5265  refreshDeleriumStateFromInventory_();
5267  const pendingId = '_pending_' + Date.now();
5269  inventoryResourceLedger = [{ '_pending': true, … 'Action': 'SELL', 'Qty': -totalQty, … }, …].slice(0, 60);
5276  cacheInventoryRows(inventoryRows, inventoryResourceLedger);   // <-- UNGUARDED: _inFlightWrites is 0 here
5281  if (status) status.textContent = 'Selling…';
5282  renderDeleriumSheetBody();
5284  gasCall('apiSellDelerium', …)                                 // gasCall bumps _inFlightWrites only now
```

At line 5276 the `gasCall` that owns the round-trip's `_inFlightWrites++` has not run yet, so (in the common single-write case) `_inFlightWrites === 0` and the cache write **executes**. It persists into `CACHE_KEY`: (a) the optimistic `_opt_<size>_<ts>` rows with `Qty: -qty` (5256–5262) — temp IDs no server row will ever reconcile — and (b) the synthetic `_pending` SELL ledger entry that carries no `Inventory ID` and, once the handler's `pendingId` filter is gone, has no counter to clear it.

State-machine consequence on **navigate-away / iOS-suspend after the tap but before the handler fires** (the scenario the run-64 fix was written for): the last state committed to localStorage is the phantom optimistic state. On the next cold open, `loadInventory` paints from that cache first (3937–3946: `inventoryRows = cachedPayload.rows; inventoryResourceLedger = cachedPayload.resourceLedger`), so the user briefly sees the crystals already deducted *and* a "SELL … (saving…)" ledger row that will never resolve on its own. It self-heals only when `apiGetInventory` returns and wholesale-replaces both arrays (3975–3976) — identical transient-but-real severity to the `receiveDelerium` bug the maintainers judged worth fixing. The fix is the same three-line bracket: `_inFlightWrites++;` before the 5276 cache, `_inFlightWrites--;` after `renderDeleriumSheetBody()` and before `gasCall`. (Note the mirror asymmetry is the tell: `receiveDelerium` guards, `sellDelerium` — copied from the same template — does not.)

#### RISK · Code.js:3089,3112 · `apiSellDelerium` writes its N per-size delerium rows and the gold-credit row as N+1 separate `appendRow` calls — a partway death (quota/timeout) leaves crystals deducted but gold un-credited (value destroyed), the same non-atomic class that `apiSplitGold`/`apiSendGoldToMember` were explicitly batched to fix

**Story: Sell crystals — failure at the server mid-write; execution-trace on the multi-row commit.** LockService gives mutual exclusion, not atomicity — the same premise the in-code comments on `apiSplitGold` (3200–3204) and `apiSendGoldToMember` (3302–3304) cite as the reason those two build every row up front and commit in **one** `setValues`. `apiSellDelerium` never got that treatment: it appends one negative-qty row **per delerium size inside the `items.map`** (3089) and then, separately, the positive gold-credit row (3112) — up to N+1 independent `appendRow` calls under the single lock. If the script dies between them (the 6-minute execution ceiling, a quota blip, a transient Sheets error), the sheet is left in an asymmetric state the client cannot detect:

- die after some size rows but before the gold row → crystals removed from stock, **no gold credited** — the party's gold value for that sale is silently destroyed;
- die between size rows → only some of the selected sizes are deducted, but the client's `sellDelerium` success/fail handler never runs, so its optimistic rows (all sizes) are neither reconciled nor reverted until the next `apiGetInventory`, which then shows a partial sale the user never intended.

Because `appendResourceLedger_` is interleaved with the row appends (3090/3113) and swallows its own errors (README Known TODO), the ledger can also end up describing a sale the inventory only partially reflects. This is the identical multi-write/no-rollback defect flagged as a class at FINDINGS:455–456 (which named `apiSendGoldToMember` and the delerium branch of `apiReceiveResource` but **not** `apiSellDelerium`); both of the gold members of that class were subsequently fixed by batching, while `apiSellDelerium` — which is worse because it mixes a *removal* and a *credit* whose partial application destroys value rather than merely duplicating a row — was left on the old pattern. Fix: build `createdItems`' rows plus the optional gold row into one array and commit with a single `getRange(startRow, 1, allRows.length, headers.length).setValues(...)` exactly as `apiSplitGold` does (3212–3214), appending the ledger entries only after the atomic inventory write.

### 2026-07-15 (run 67) — Sections audited: 3 (Code.js 1401–2100: the "inventory write — add, edit, delete" slice — the shared write plumbing this section actually holds: profile/identity save (`saveUserProfile_` 1433, `validateCharacterChoice_` 1478), access gates (`requireTreasurer_` 1536, `requireAllowedUser_` 1596, `requireAdminUser_` 1628), the error mapper `publicApiError_`/`publicValidationError_` (1646/1658), the input validators `validateId_`/`validateText_`/`validateQuantity_`/`validateMoney_`/`normalizeOptionalMoney_` (1676–1731), the client sanitizers (1733–1799), and the inventory row engine every add/edit/delete/combine call routes through — `getInventoryRowObjectById_` 1851, `writeInventoryRow_` 1864, `findMatchingInventoryRow_` 1869, `mergeIntoExistingRow_` 1899, `classifyQuickEdit_` 1928, `auditWrite_` 1956, `appendResourceLedger_` 1986, `deleteResourceLedgerRowsForInventory_` 2013 — read out to the write endpoints that consume them: `apiAddInventory` 2354, `apiAddCustomInventory` 2470, `apiUpdateInventory` 3345, `apiDeleteInventory` 3436, `apiCombineInventoryItems` 3547, and `findInventoryRowById_` 4048)

> **Stories traced** (happy → failure-at-step → navigate-away → friction, execution-trace + state-machine on every write path): **Add library item** (search→select→qty/holder/notes→Add: client `addInventoryItem` Index:8734 optimistic `_opt_` row 8784 + `clearAddForm` + `gasCall('apiAddInventory')` 8900 → server value-fallback 2386 / `findMatchingInventoryRow_` 1869 / `mergeIntoExistingRow_` 1899 auto-combine → `onAddSuccess` 8814 removes optId, `primeInventoryCacheAfterAdd` 3243 reconciles by Inventory ID, `findDuplicateInventoryCandidate`/`showCombineChoice` 8850); **Add custom item** (`apiAddCustomInventory` 2470 — no value fallback, `validateMoney_` directly); **Add-another from description sheet** (`_confirmDescActionAdd_` Index:7380 — client `existingMatch` in-place increment 7403 vs server auto-combine, revert-on-`!ok` 7448); **Edit inventory item** (`selectInventoryItem` 6927 → `syncInventoryEditorFieldsFromRow` 7892 + rollup `readOnly` re-lock 6942 → `saveInventoryEdits` 7987 → `apiUpdateInventory` 3345 → `updateInventoryRowLocally` 8023, non-optimistic, button re-enable on both branches); **Delete inventory item** (swipe two-tap arm `handleInventoryDeleteActionById` 8061 → `deleteSelectedInventory` 8096 qty>1 confirm gate 8111, optimistic filter/decrement + `previousRows` revert 8136, `apiDeleteInventory` 3436 / `apiAdjustInventory` decrement 8185); **Combine duplicate** (`confirmCombineInventoryItem` Index:3729 → `apiCombineInventoryItems` 3547 delete-source-then-rewrite-target, `removedId`/merged `item`); plus the cross-cutting **Collaborative sync interference** / **Tab-switch in-flight** / **iOS background-foreground** stories through `gasCall`/`_inFlightWrites`/`drainPendingForeignReload_` 3035/3046 and `cacheInventoryRows` in-flight guard 3232. Per protocol I re-verified prior in-range items landed and did NOT re-report them: `mergeIntoExistingRow_` now merges Notes/Faction (run-64 BUG, FINDINGS:365, present 1910–1922); the run-65 `publicValidationError_` allowlist extension (1665–1668, both prefix groups present); the `editQty`/`sheetEditQty` `readOnly` reset (7921–7924); locks released in `finally` on every add/edit/delete/combine path; `descActionInFlight` never-reset RISK (still open, FINDINGS run-64) and the `apiGetInventory`/`fillMissingInventoryRarity_` unlocked-backfill RISK (still open, run-66 top) both re-confirmed unchanged. **One new finding** — it is a direct counterexample to the run-64 "parity holds" trace (FINDINGS:158–162) and the run-65 Note "no double-count" (FINDINGS:685): those reasoned the client's 5-tuple `existingMatch` always agrees with the server's, but a blank Value GP makes them disagree.

#### RISK · Code.js:2386 / Index.html:7403 · `apiAddInventory`'s blank-Value-GP → library-default fallback breaks the client/server auto-combine-match parity, so re-adding an item that carries a blank value double-counts (client increments an existing row the server never touched, then prepends the server's new row)

**Story: Add library item / Add-another from description sheet — happy path + navigate-away; execution-trace + state-machine on the auto-combine write.** The whole "no double-count on add" guarantee that runs 64 and 65 signed off on (FINDINGS:158–162 "server combine-match parity with the client `existingMatch` 5-tuple … so `_confirmDescActionAdd_`'s in-place increment doesn't double-count"; FINDINGS:685 Note) rests on one invariant: **the 5-tuple the client matches on (Item · Category · Rarity · Holder · Value GP) is byte-for-byte the 5-tuple the server matches on.** `apiAddInventory` violates it for the Value GP element.

Server (`apiAddInventory` 2386–2391):
```
const requestedValue = payload && payload.valueGp;
const valueGp = requestedValue === '' || requestedValue === null || requestedValue === undefined
  ? normalizeOptionalMoney_(libraryItem.valueGp)   // <-- blank input is REPLACED by the library default
  : validateMoney_(requestedValue);
```
So when the client sends a **blank** `valueGp`, the row the server builds — and therefore the row `findMatchingInventoryRow_` (1869, value compare at 1893 `String(toIdx(valCol)) !== targetVal`) searches for — carries the **library's** value, not blank.

Client (`_confirmDescActionAdd_` Index:7392 / 7403–7409):
```
valueGp: item['Value GP'] || '',                      // 7392 — blank stays blank
...
const existingMatch = inventoryRows.find(r => … &&
  String(r['Value GP'] || '') === String(payload.valueGp || ''));   // 7408 — matches on BLANK
```
The client's `existingMatch` looks for a row with a **blank** value; the server looks for a row with the **library** value. When the re-added item's stored Value GP is blank but its Library Item ID resolves to a non-blank library value, the two searches target different rows:

1. Item "Potion of X" sits in inventory with **blank** Value GP and a valid Library Item ID (reachable today: add it, then **Edit → clear the Value GP field →** `apiUpdateInventory` 3374 `validateMoney_('')==''` writes blank while preserving Library Item ID 3378; the library item still resolves with a non-blank value).
2. User opens its description sheet → **Add** → qty 1 → Confirm. `_confirmDescActionAdd_` finds `existingMatch` (the blank-valued row is right there), sets `optId = null`, and **increments that row in place** (7413–7417), caches, renders. The user sees the count go up by 1.
3. Server: `requestedValue === ''` → `valueGp = normalizeOptionalMoney_(libraryItem.valueGp)` = the library value (say 50). `findMatchingInventoryRow_` searches for a row with Value GP = 50 → none exists (every matching row is blank) → **appends a brand-new row** with value 50, `combined:false`, returns `res.item` = that new row with a fresh Inventory ID.
4. `onAddSuccess` (7445): `optId` is null so no optimistic row is removed; `primeInventoryCacheAfterAdd(res.item)` (3243) sees an Inventory ID **not** present in `inventoryRows` → **prepends it** (3251). Net result: the original row is now +1 (client optimistic) **and** a second phantom row of +1 exists (server) — the item is **double-counted and split across two rows**, and no reload is scheduled, so it persists until the next foreign-sync or manual refresh. (`descActionInFlight` clears normally; state is otherwise clean — this is a data-integrity bug, not a stuck state.)

The main **Add library item** flow (`addInventoryItem` 8734) is largely shielded from the *double-count* because it never pre-increments — it always drops an `_opt_` row keyed by a temp Inventory ID and `primeInventoryCacheAfterAdd` reconciles strictly by Inventory ID, so a server auto-combine lands on exactly one row (this is the case runs 64/65 traced). But the same fallback still bites it two softer ways: (a) if the user deliberately **blanks** the value to record a valueless item, the server silently overrides their choice with the library default, and the optimistic row (blank) flips to the library value after the round-trip — the user's intent is discarded with no feedback; (b) that add/edit asymmetry is itself inconsistent — `apiUpdateInventory` (3374) and `apiAddCustomInventory` (2501) both honor a blank value (`validateMoney_('')===''`), only `apiAddInventory` rewrites it.

Fix options: (1) make the client match the server — have `_confirmDescActionAdd_` (and the main add's optimistic value) resolve the same library-default fallback before computing `existingMatch`, so both sides key on the identical value; **or** (2) drop the server fallback and treat blank as blank in `apiAddInventory` (matching `apiUpdateInventory`/`apiAddCustomInventory`), pushing the "default from library" decision to the client form where the user can see and override it; **or** (3) since the server already returns `combined:false` and a real Inventory ID, have `_confirmDescActionAdd_` reconcile like the main flow — only ever adjust the row identified by `res.item['Inventory ID']` in the success handler and undo the optimistic in-place increment when the server did not combine. Option (2) is the cleanest: it removes the divergence at the root and restores the parity the prior "no double-count" traces assumed.

### 2026-07-14 (run 66) — Sections audited: 2 (Code.js 701–1400: the "auth, character, inventory read" slice — tail of the shared `apiSellInventoryBatch`/`apiGiveInventoryBatch` FIFO write path (701–862), the roster read `apiGetCharacters` (864), identity resolution `apiGetMyCharacter`/`apiSetMyCharacter`/`apiForgetMyCharacter` (932–978), the small read endpoints `apiGetCategories`/`apiGetQuickAddItems`/`apiLogUse` (984–1029), classification/general helpers (1067–1346), and the identity/access/validation helpers at 1348–1400 continuing into `saveUserProfile_`/`validateCharacterChoice_`/`requireTreasurer_`/`requireAllowedUser_`/`publicValidationError_` (1433–1668) that gate every story — read out to the main inventory read `apiGetInventory` (2103) + `fillMissingInventoryRarity_` (3983) it calls, and to the client consumers `loadCharacters`/`loadMyIdentity`/`applyIdentity`/`confirmIdentity`/`loadInventory`)

> **Stories traced** (happy → failure-at-step → navigate-away → friction, execution-trace + state-machine on every write path): **First open / identity picker** (`loadMyIdentity` Index:4723 → `readCachedIdentity`→`applyIdentity` optimistic, then `loadFallbackCharacterIdentity` → `apiGetMyCharacter` 932 [temp-key profile → email → client hint], `showIdentitySheet` 4748 fed by `loadCharacters` 3205 → `apiGetCharacters` 864, `confirmIdentity` 4787 → `apiSetMyCharacter` 961 → `saveUserProfile_` 1433 → `validateCharacterChoice_` 1478); **App-load inventory read** (`loadInventory` Index:3909 → `apiGetInventory` 2103 → `fillMissingInventoryRarity_` 3983 → `sanitizeInventoryForClient_`; cache/in-memory paint + signature-diff re-render + `_inFlightWrites` foreign-reload defer at 3964); **Holder dropdowns** in Add/Edit/Give/Split (`populateCharacterSelectors` 3185, consumers 5517/5552/6098/7224 — all read `characterOptions`); **Give item to character** (`apiGiveInventoryBatch` 767 server path — multi-row drain-then-merge, verified correct; client single-row limitation is the documented README TODO, not re-reported); **Treasurer gating** (`requireTreasurer_` 1536 hint fallback). Re-verified prior in-range fixes landed and did NOT re-report: the run-65 `publicValidationError_` allowlist-drift BUG (still open, FINDINGS top of run 65 — `Cannot remove…`/`Amount…`/`No delerium…` still map to "Request failed."; unchanged in current code), the run-58/61 `apiGetMyCharacter`-failure-bounces-cached-identity RISK (FINDINGS:196, still open), the run-59 sync-poll-resume RISK (FINDINGS:213, still open), and the FIXED identity-picker-brick retry (FINDINGS:1312 / 737). **Two new findings** — both are read-path defects that the write-path-focused prior runs skipped.

#### RISK · Code.js:2103 / 4039 · `apiGetInventory` (a *read* endpoint) triggers an unlocked bulk write of the whole Rarity column via `fillMissingInventoryRarity_` — it can race a concurrent structural write and silently misalign rarities

**Story: App-load inventory read + Collaborative-sync foreign reload (cross-cutting).** `apiGetInventory` (2103) takes **no LockService lock** — correct for a pure read, except it isn't a pure read. Before reading rows it calls `fillMissingInventoryRarity_(sheet, ensureInventoryHeaders_(sheet))` (2114), which back-fills the `Rarity` column for legacy inventory rows that carry a `Library Item ID` but blank `Rarity` (items added before the equipment-library import — see README Known TODOs). When any such row exists, the helper does `getValues` on the Library-ID+Rarity columns (4000), builds a `rarities` array **indexed by physical row position**, then writes it back with a single `inventorySheet.getRange(2, rarityCol, numRows, 1).setValues(rarities)` (4038–4040) — a bulk mutation of up to `numRows` cells, with `numRows` frozen at read time (3996).

Because the enclosing `apiGetInventory` holds no lock, this read-compute-write sequence interleaves freely with the lock-holding structural writers in this file and the sibling slices — `apiSellInventoryBatch` (`sheet.deleteRow`, 746), `apiGiveInventoryBatch` (`deleteRow`/`appendRow`, 808/840), the quick-adjust decrementers, delete, and combine. The document lock those writers take does **not** protect the sheet from `apiGetInventory`, since `apiGetInventory` never asks for it. Trace of the corruption:

1. Party has one un-backfilled legacy row, so `fillMissingInventoryRarity_` will write. Inventory has rows R2…R10.
2. User B loads inventory → `apiGetInventory` → `fillMissingInventoryRarity_` reads columns at 4000 (`numRows = 9`), computes the `rarities` array for the current R2…R10 order, and starts resolving equipment rarities (4015–4027 — another spreadsheet open + read, widening the window).
3. Meanwhile User A's `apiSellInventoryBatch` (holding the doc lock) `deleteRow(5)`. The sheet now has 8 data rows; everything below the deleted row shifts up by one.
4. B reaches `setValues(getRange(2, rarityCol, 9, 1), rarities)` at 4039. It writes B's **stale, pre-delete** 9-element array onto the **post-delete** rows: every row below the deletion point receives the rarity of the item that used to sit one row lower, and the 9th write lands on a now-empty/other row. The `Rarity` column is silently misaligned — item X wearing item Y's rarity — and it persists to the sheet and to every subsequent client load. No error is surfaced (the write "succeeds").

Aggravating factor specific to the sync story: `loadInventory`'s in-flight guard (`if (_inFlightWrites > 0) { pendingForeignReload = true; … return; }`, Index:3964) only suppresses **client-side application** of the result — the `apiGetInventory` call has already fired and runs to completion server-side. So even the "defer the foreign reload while my write is in flight" path still executes the racy unlocked backfill on the server concurrently with the local lock-holding writer. The precondition (an un-backfilled row must exist) is transient — once every legacy row is filled the helper early-returns at 4013 — which is exactly why the write-path-focused runs (section-7 review at FINDINGS:1523 read `fillMissingInventoryRarity_` and found nothing) missed it: the bug lives in the *coupling* between a lock-free read endpoint and a bulk mutating helper, not in either piece alone. Fix: either acquire the document lock in `apiGetInventory` around the backfill (matching the writers), or move the backfill off the read path entirely (a one-shot admin/menu utility), or make the write position-safe by keying rarities to `Inventory ID` and using a text-finder per row instead of a positional `setValues`.

#### RISK · Index.html:3205 · A transient `apiGetCharacters` failure at boot permanently degrades every character dropdown for the whole session when a cached identity is present — no error, no retry, no self-heal (the run-46/50 retry fix only covers the identity-sheet-*active* path)

**Story: Add library item (holder), Edit inventory item (holder), Give item to character, Split gold (member display), Pay gold (character routing).** `characterOptions` is populated in exactly one place — `loadCharacters`' success handler (Index:3208) — and `loadCharacters` is called from exactly one place at boot (Index:9004) plus the retry button it renders (3217). Its failure handler (3212–3218) shows that retry button **only** `if (sheet && sheet.classList.contains('active') && body)` — i.e. only when the first-open identity splash is up.

The common path is a **returning user with a valid cached identity**: `loadMyIdentity` (4723) reads the cache and `applyIdentity(cached)` runs immediately, so the identity sheet is **never shown**. If the parallel boot `apiGetCharacters` call (fired at 9004, independent of identity) hits `withFailureHandler` — a GAS cold-start, quota blip, or network drop, the exact flaky conditions the identity cache exists to survive — the handler's guard is false (sheet not active), so it `console.error`s and does nothing else. `characterOptions` stays `[]` for the rest of the session. Downstream, with no user-visible symptom and no recovery:

- `populateCharacterSelectors` (3185) renders the Add/Edit **Holder** `<select>` with only `Party / shared` — the user cannot assign any item to a character.
- The **Give to…** picker (5552 / 7224) offers only Party Pool — the entire "Give item to character" story is unreachable.
- Split-gold member list (5517) and Pay-to-character routing (6098) compute against an empty roster.

There is no re-fetch anywhere: `loadCharacters` is not re-invoked on tab switch, on the 20 s sync poll, on inventory reload, or on sheet open — only at boot and via the retry button that never renders here. So the degradation lasts until a full page reload, silently. This is distinct from the prior finding: the run-46/50 RISK "loadCharacters failure bricks the first-open identity picker" (FINDINGS:1312 / 737, marked **FIXED** by adding the in-sheet retry button) fixed only the sheet-*active* case; and the passing mention at FINDINGS:3319 ("degrades to 'Party / shared' only") mis-cross-references FINDINGS:205 (which is about `apiGetMyCharacter`, a different call) and treats the degradation as acceptable rather than as an unrecovered failure. Fix: give `loadCharacters`' failure handler a session-level self-heal independent of the identity sheet — e.g. a bounded retry with backoff, or a re-fetch hook on tab-switch/`populateCharacterSelectors` when `characterOptions` is empty, and/or surface a non-blocking status so the empty Holder dropdown isn't silent.
...
Every prior `#### BUG/RISK/IDEA` finding that had not been marked `✅ FIXED` was re-verified against the **current** `src/Code.js` / `src/Index.html` (finding line numbers are stale, so each was relocated by symbol). Result: **32 findings were already fixed in earlier commits but never marked**, **2 are partially addressed**, **17 remained genuinely open**. Fixed/partial headers above have been tagged inline (`✅ FIXED (reconciled 2026-07-13)` / `⚠️ PARTIAL (reconciled 2026-07-13)`) so future runs reference — not re-report — them. `#### Note` entries are positive clean-trace observations and were left as-is.

> **Follow-up fix (2026-07-13, same session):** 5 of the 17 open BUGs were then fixed in code — `publicValidationError_` allowlist drift (Code.js, extended the allowlist with anchored domain-message prefixes so "Cannot remove more than…", "Amount is too small…", "DM is not a party member…", "Only matching items…", "Source and target…", "No delerium…", "No characters found", "Could not identify…" reach the client while internal engine errors still mask to "Request failed."); `editQty`/`sheetEditQty` readOnly latch (`syncInventoryEditorFieldsFromRow` now clears `readOnly=false` on every editor open before the rollup guard re-locks); `receivedGold` `!res.ok` branch now calls `renderInventory()` so the dashboard drops the reverted phantom gold; `setNoteArchived_` failures now report to in-view `#notesStatus` instead of the off-screen `#mainStatus`; `loadFallbackCharacterIdentity` failure handler now `return`s early when a cached identity is already applied, so a transient `apiGetMyCharacter` error no longer wipes identity and force-shows the picker. Those 5 headers are tagged `✅ FIXED (2026-07-13, this session)`. **12 open items remain** (see updated table below).
...
**❗ Still OPEN (12)** — the actionable backlog (5 BUGs from the run-65 list were fixed this session — see follow-up note above):
...
#### BUG · Index.html:6884,7829 · `editQty`/`sheetEditQty` `readOnly` is set for a rolled-up item but never reset — the quantity field stays locked for every subsequent single-addition edit in the session (with no hint explaining why)  ✅ FIXED (2026-07-13, this session)

**Story: Edit inventory item — happy path on a single-addition item after previously editing a rolled-up one.** `selectInventoryItem` (6864, the swipe→Edit / desktop card-tap entry into the full editor) guards multi-addition items: when `rollupCount > 1` (the same name/holder/value/etc. exists as several separate `PARTY_INVENTORY` rows) it makes the qty field non-editable and shows a hint —

```
6880  if (rollupCount > 1) {
6882    ['editQty', 'sheetEditQty'].forEach(id => { const el = ...; if (el) el.readOnly = true; });
6886    ['editStatusMessage', 'sheetEditStatusMessage'].forEach(... innerHTML = hint ...);
6890  }
```

The intent is correct: a rolled-up qty can't be edited from the flat editor (you must use the description sheet's FIFO stepper). **But `el.readOnly = true` is never paired with an `el.readOnly = false` anywhere in the file** — a full-file grep finds only four `readOnly` writes (the two gold/delerium ledger-edit toggles at 4783/6362/6380, which correctly reset, and this one at 6884, which never does). `syncInventoryEditorFieldsFromRow` (7829), which runs on *every* editor open at 6877 and only sets `.value`/`.textContent`, does not touch `readOnly`. So the property latches `true` on the DOM `<input>` for the rest of the session.

Trace: (1) party has "Potion of Healing" added twice (two additions → `rollupCount` 2) and "Rope" added once (`rollupCount` 1). (2) User swipes the Potion card → **Edit** → `selectInventoryItem` runs the `rollupCount > 1` branch → `editQty.readOnly = true` + `sheetEditQty.readOnly = true`, hint shown. (3) User closes the editor (Save / Cancel / `closeInventoryPanels`) — none of these reset `readOnly`. (4) User swipes the **Rope** card → **Edit** → `syncInventoryEditorFieldsFromRow` repaints the fields (and blanks `editStatusMessage`/`sheetEditStatusMessage` to `''` at 7841/7852), then `rollupCount === 1` so the guard block is **skipped**. (5) The editor opens for the single-addition Rope with its qty field **still `readOnly` from step 2** — the user cannot change the Rope's quantity at all, and because `syncInventoryEditorFieldsFromRow` already cleared the status line, **there is no hint on screen explaining the lock**. It looks like a dead input. The only recovery is a full page reload.

Worse than a stale hint: the field is silently uneditable for the core "change an item's quantity" flow, and it affects *all* subsequent flat-editor edits (single-addition or not) once any rolled-up item has been edited once — a very common sequence in a session where duplicates accrue. Saving still works (`getInventoryEditorValues` reads `.value` regardless of `readOnly`, 7893/7880), so the user just re-saves the unchanged qty, believing the app dropped their edit.

Fix: reset `readOnly` on the non-rollup path — either add an `else` to the 6880 guard that sets `editQty.readOnly = false` and `sheetEditQty.readOnly = false`, or (cleaner, covers every editor-open entry point including the description-sheet Edit and `openFullEditorFromQuick` 7736) unconditionally clear `readOnly = false` for both ids inside `syncInventoryEditorFieldsFromRow` (7829) before the caller's rollup check re-applies it. Mirrors the ledger-edit toggle, which already resets `amountInput.readOnly = false` on `cancelLedgerEdit` (6380) and the gold-sheet open (4783).

### 2026-07-11 (run 63) — Sections audited: 11 (Index.html 6001–7500: description sheet + its Add/Sell/Give/Remove sub-sheet flows, inventory-group build/classify/rollup, the shared `apiSellInventoryBatch`/`apiGiveInventoryBatch` FIFO-drain write path, and the quick-edit currency/delerium panel — plus the collaborative-sync/tab-switch/iOS-foreground cross-cutting stories where they cross these in-flight writes)

> **Stories traced** (happy → failure-at-step → navigate-away → friction, execution-trace + state-machine on every write path): **View item details** (`openInventoryDescription` 6911 → async `apiGetEquipmentItem` with the `capturedId`/`selectedInventory` staleness guard 6960–6974, `itemCache` only-cache-on-ok 6965), **Sell item** (`openDescActionSheet('sell')` 7099 → `confirmDescAction` 7207 → two-tap `_requireSellConfirm` 3029 → `_confirmDescActionSell_` 7410 via `apiSellInventoryBatch`), **Give item to character** (`openDescActionSheet('give')` → `selectDescGiveTarget` 7199 → `_confirmDescActionGive_` 7216 via `apiGiveInventoryBatch`), **Remove item** (`_confirmDescActionRemove_` 7484, goldAmount=0 `apiSellInventoryBatch`), **Add library/custom item** re-add-from-description (`_confirmDescActionAdd_` 7317, `existingMatch` in-place increment vs `apiAddInventory` auto-combine 2349/`findMatchingInventoryRow_` 1864/`mergeIntoExistingRow_` 1894), the FIFO-drain qty steppers (`getDescRemoveTotalQty_` 6989, `stepDescAction` 7188, `renderDescActionBody_` 7129), **Quick-adjust currency/delerium** (`openQuickEditPanel` 7593 → `apiGetCurrencyQuickEdit` 3665 → `confirmQuickEdit` 7746, size-reclassify confirm `onQuickSizeChange` 7680), **Undo last pay** where it re-enters this range (`undoGoldPay` 5366 → `undoResourcePay` 6723), and the cross-cutting **Collaborative sync interference** / **Tab-switch in-flight** / **iOS background-foreground** stories across the `gasCall`/`_inFlightWrites`/`drainPendingForeignReload_` layer (2991–3007) and `renderInventoryDashboard`/`buildInventoryGroups`/`getInventoryGroupKey`/`rollupInventoryRows` (4088/6491/6520/6599). Per protocol I re-verified prior fixes in-range landed and did NOT re-report them: the description-Sell/Remove `descActionInFlight` re-entry guard (run-37 RISK, FINDINGS:2251, present at 7411/7485); the `apiSellInventoryBatch`/`apiGiveInventoryBatch` gold-before-delete ordering (run-19 RISK 3503, confirmed at Code.js 717–753); the `_inFlightWrites++/--` bracketing that all four descAction writes now inherit via `gasCall` (run-19/18 BUGs 3494/3519); server combine-match parity with the client `existingMatch` 5-tuple (Item+Category+Rarity+Holder+Value GP) so `_confirmDescActionAdd_`'s in-place increment doesn't double-count normally (auto-combine keeps ≤1 physical row per exact 5-tuple). Confirmed **dead / harmless, not re-counted**: the legacy pinned-remove machinery (`confirmDescRemove` 7018, `stepDescRemoveQty` 7012, `updateDescRemoveRow_` 7002 targeting `descRemove*` ids that no longer exist in the sheet — superseded by the sub-sheet stepper); the dashboard inline resource-breakout pay UI (`renderResourceBreakout` 6208 / `payResource` 6646 / `toggleResourceBreakout` 4596 — the dashboard now renders only the two sheet-opening stat buttons, 4106–4116); the section-header/`inventoryExpandedGroups`/`toggleInventoryGroup` toggle machinery neutered by the 2026-07-10 continuous-list trial (`renderInventoryGroup` 6575 `void isOpen; void totalQty`). **Two new findings:**

#### RISK · Index.html:6968,2968 · `descActionInFlight` never resets outside its own callbacks — a dropped call latches ALL description-sheet Add/Sell/Give/Remove permanently (the un-fixed twin of the quickEditInFlight RISK, FINDINGS:3481)

**Story: Sell / Give / Remove / re-Add item from the description sheet; cross-cutting iOS background-foreground and tab-switch-in-flight.** `descActionInFlight` (declared 2968) is the single guard shared by all four description-action confirm handlers: each opens with `if (descActionInFlight) return;`, sets it `true` immediately before its `gasCall`, and clears it **only** inside that call's success and failure handlers (`_confirmDescActionGive_` 7217/7294/7308, `_confirmDescActionAdd_` 7318/7383/7401, `_confirmDescActionSell_` 7411/7461/7475, `_confirmDescActionRemove_` 7485/7538/7550). Nothing else ever resets it — `openDescActionSheet` (7099) and `closeDescActionSheet` (7124) leave it untouched, and the `visibilitychange` foreground handler (8933) only re-checks the sync poll. So if a `gasCall` callback never fires — the iOS GAS webview suspending JS mid-round-trip, or `google.script.run` being torn down when the webview is backgrounded/killed before the server responds — the flag latches at `true`.

Trace: (1) user opens an item's description sheet → taps **Sell** → sub-sheet opens → enters gold → double-taps Confirm → `_confirmDescActionSell_` closes both sheets, optimistically decrements the rows, sets `descActionInFlight = true`, fires `apiSellInventoryBatch`. (2) iOS backgrounds the webview (user switches apps / screen locks) before the response returns; the callback is dropped on resume. (3) `descActionInFlight` stays `true` forever. (4) For the rest of the session, **every** description-sheet action — Sell, Give, Remove, and re-Add — early-returns silently at its guard: the Confirm button (and, for Sell/Remove, the second tap of the two-tap `_requireSellConfirm` which already returned `true` and disarmed) does nothing, no error, no status. The description sheet is effectively read-only until a full reload, and the user has no way to tell why their taps are ignored.

This is the exact class already logged and reportedly closed for `quickEditInFlight` (FINDINGS:3481, "never resets on a lost callback — quick-adjust becomes permanently dead until reload") — whose suggested fix was to reset the flag in the open/close handlers and/or restore it on `visibilitychange` foreground. `descActionInFlight` gates four core item-management flows (a strictly larger blast radius than the single quick-edit flow) and never received that treatment; note `closeQuickEditPanel` even documents (7719–7723) the deliberate choice not to reset its flag on close, which is why the open/foreground reset is the intended remedy — the description path has neither. Related, the shared `_inFlightWrites` counter has the same latch shape on a dropped callback (it would stick > 0, so `drainPendingForeignReload_` at 3003 never fires and every subsequent foreign inventory change is deferred into `pendingForeignReload` and silently never applied — collaborative sync dies for the session); that counter is cross-cutting rather than section-11-local, but the description writes are one of its feeders. Fix: reset `descActionInFlight = false` (and reconcile `_inFlightWrites`) on `visibilitychange` foreground and/or in `openDescActionSheet`, mirroring the quick-edit remedy.

#### IDEA · Index.html:7439,7523 · Description-sheet Sell and Remove close both sheets *before* the round-trip and give no success feedback, so a failure lands only on the easily-missed `#mainStatus` while the pre-built in-sheet `#descActionStatusRow` goes unused

**Story: Sell item / Remove item — friction + failure-at-step.** All four descAction confirm handlers call `closeDescActionSheet(); closeDescriptionSheet();` **before** the server call (Sell 7439–7441, Remove 7523–7524, Give 7278–7279, Add 7375–7376), then report failures via `setMainStatus(...)`. Unlike Give (which at least paints an in-progress `"Giving Nx … to Y…"` success line, 7284) and Add (`"… added."`, 7398), **Sell and Remove emit nothing on success** — the item silently decrements/vanishes from the list with no confirmation that the sale/removal actually committed, and no indication of how much gold was received. On failure, `_confirmDescActionSell_` (7466/7479) and `_confirmDescActionRemove_` (7543/7554) revert the optimistic rows and write the error to `#mainStatus` — the shared banner at the top of `<main>` that, per the run-50/61 findings, routinely scrolls off-screen (the whole reason `#notesStatus` was created for the Notes tab). A treasurer who sells an item, glances away, and comes back sees the item quietly reappear with the error already scrolled past — the same silent-revert symptom those runs flagged for notes, now on the money-losing sell path. Compounding it, the sub-sheet already contains a purpose-built in-context status element, `#descActionStatusRow` (HTML 2873, initialised empty/hidden by `openDescActionSheet` 7111–7112), that **no confirm handler ever writes to** because they all close the sheet first. Suggested UX: keep the descAction sub-sheet open with a disabled/"Saving…" Confirm button until the round-trip resolves (mirroring the Notes form and the combine sheet), surface success ("Sold 3× Potion for 30 gp") and failure in `#descActionStatusRow`, and close only on success — so a failed sell/remove stays put and is retryable in place instead of reverting behind a banner the user won't see. Also worth aligning: Sell-for-Gold does not optimistically reflect the received gold on the dashboard `Gold = X` total (it only appears once `primeInventoryCacheAfterAdd(res.goldItem)` runs on success, 7469), inconsistent with `receivedGold`/`confirmPayWithReason` which insert an optimistic gold row so the total moves instantly.

### 2026-07-10 (run 62) — Sections audited: 10 (Index.html 4501–6000: gold sheet + pay/split/undo, delerium sheet + receive/sell counters, ledger note-edit, give/sell-item + sell-batch, and the collaborative-sync interaction with all of these while a resource sheet is open)

> **Stories traced** (happy → failure-at-step → navigate-away → friction, execution-trace + state-machine on every write path): **Receive gold** (`receivedGold` 5372), **Pay gold** (`openPayReasonSheet` 5452 → `confirmPayWithReason` 6036 via `apiDepleteResource`/`apiSendGoldToMember`), **Split gold evenly** (`splitGold` 6145), **Undo last pay** (`undoGoldPay` 5366 → `undoResourcePay` 6728), **Edit ledger note** for both resources (`selectLedgerRowForEdit` 6342, `cancelLedgerEdit` 6373, `updateLedgerNoteFromBottom` 6390, `renderResourceLedger` 6293); **Receive crystals** (`receiveDelerium` 5050) and **Sell crystals** (`sellDelerium` 5163, incl. the 0-gp inline confirm) with the counter state machine (`adjustDeleriumSell` 4962 / `adjustDeleriumReceive` 5009 / `updateDeleriumButtonStates` 5029 / `refreshDeleriumStateFromInventory_` 5152); **Give item to character** (`openGiveItemSheet` 5490 → `giveItemToCharacter` 5980), **Sell item** (`openSellItemSheet` 5520 → `confirmSellItem` 5541), and the treasurer **Sell Items batch** (`openSellBatchSheet` 5617 → `confirmSellBatch` 5906, plus `copyPartyPoolInventory` 5812); and the cross-cutting **Collaborative sync interference** / **Tab-switch in-flight** / **iOS background-foreground** stories where they cross the open gold/delerium sheets (`gasCall`/`drainPendingForeignReload_` 2991/3002, `pollSync` 4121, `loadInventory` 3855). This section was last swept by run 37 (FINDINGS:2252) and the render/scope layer by run 55/57; per protocol I re-confirmed those fixes landed — `goldSheetMutated` is now set in the optimistic phase not just on success (run-55 BUG 2324 closed at 5408/6084/6173); optimistic gold rows now inserted in `receivedGold`/`confirmPayWithReason` (run-53 Fix 3, present at 5396/6064); `undoGoldPay` re-renders the ledger (5368); `_clearSellConfirm`/`_requireSellConfirm` arm-to-confirm on sells (3023–3040) verified; delerium 0-gp confirm (5178) and the "gold field only logged by Sell" guard (5058, run-37 RISK 283) verified present. The `isPartyScope`-hoisting and DM-scope Got-Paid items (run-49/53) re-read clean. Sell-batch FIFO drain, generation-counter auto-close (5967), and `copyPartyPoolInventory` clipboard fallback re-verified clean (already logged / benign, not re-counted). **Two new findings:**

#### RISK · Index.html:3921,5293,4882 · An open Gold or Delerium sheet shows a stale ledger + header total after a collaborator's write triggers `loadInventory(true)` — only the inventory list behind the sheet is re-rendered

**Story: Collaborative sync interference / Receive gold / Edit ledger note — a foreign resource write lands while the user is sitting on an open Gold or Delerium sheet.** The sync layer's job is to keep every view fresh, and it does so for the inventory tab — but the gold/delerium *sheets* render their ledger and header total only when their own `renderGoldSheetBody` (5293) / `renderDeleriumSheetBody` (4882) is explicitly called. `loadInventory(true)` never calls either. So when a foreign inventory change is applied, the open sheet silently goes stale.

Trace (idle-sheet path): (1) Treasurer opens the Gold sheet → `renderGoldSheetBody` paints the ledger from `inventoryResourceLedger` and sets the title to `Gold = X gp` (5324). `_inFlightWrites === 0`. (2) Another player records a Got Paid on their client, bumping `SYNC_INVENTORY`. (3) The 20 s `pollSync` (4121) fires: `res.inventory.by !== syncClientId` and `_inFlightWrites === 0`, so the `else` arm runs `syncState.inventory = res.inventory` and `loadInventory(true)` (4132). (4) `loadInventory` repaints the inventory list *behind* the sheet (3868/3921) and replaces `inventoryRows` + `inventoryResourceLedger` with the server snapshot that now includes the foreign entry (3913–3914) — but it never calls `renderGoldSheetBody`. (5) The open Gold sheet keeps showing the **pre-sync** ledger and the **pre-sync** `Gold = X gp` total. The treasurer reads a stale party balance and stale history for as long as they stay on the sheet — exactly when a treasurer is most likely to be deciding whether to Pay or Split.

The **drain path is worse**: if the local user has their *own* write in flight when the foreign change arrives, `pollSync` sets `pendingForeignReload` (4127); the local write's success handler renders the sheet with only the local user's new entry, then `drainPendingForeignReload_` (3002) fires `loadInventory(true)`, which overwrites `inventoryResourceLedger` with the full server ledger (local + foreign) but again does not re-render the sheet — so the sheet is left showing a ledger that is missing the foreign entry it just loaded into memory.

Not data loss and self-healing: any subsequent sheet render fixes it — tapping a scope pill (`setGoldSheetScope` 5791 → `renderGoldSheetBody`), starting/finishing any write, or closing and reopening the sheet. But until then the displayed financial total is wrong, which is precisely the failure the poll exists to prevent. This is the gold/delerium-sheet analogue of the description-sheet staleness already logged as Note (FINDINGS:2242, run 37) — that Note explicitly scoped itself to the description sheet and did not cover the resource sheets. Fix: have `loadInventory`'s success handler (and `drainPendingForeignReload_`) re-render whichever resource sheet is currently `.active` — e.g. after the `renderInventory()` at 3921, `if (document.getElementById('goldSheet').classList.contains('active')) { renderGoldSheetBody(); renderGoldSheetButtons(); }` and the equivalent for `deleriumSheet` — guarding against a `ledgerEditTarget` in progress so an in-flight note edit isn't clobbered.

#### BUG · Index.html:5428 · `receivedGold` server-error branch drops the phantom Gold row from state but never calls `renderInventory()`, so the inventory dashboard keeps showing gold that was never received  ✅ FIXED (2026-07-13, this session)

**Story: Receive gold — failure at the server call (server returns `{ok:false}`) after the user has returned to the inventory list.** `receivedGold` (5372) optimistically prepends a positive-Qty Gold row (`optGoldId`, 5396) to `inventoryRows` so the dashboard's `Gold = X gp` reflects the payment immediately. Its `removePending()` (5410) deletes that row from `inventoryRows` on settle. In the **success handler's `!res.ok` branch** (5423–5429) it calls `removePending()` and `renderGoldSheetBody()` and then `return`s — but unlike the thrown-error handler (5447) and unlike `confirmPayWithReason`'s matching `!res.ok` branch (which calls `renderInventory()` at 6102), it never calls `renderInventory()`.

Trace: (1) User taps **Got Paid** → optimistic `+amount` Gold row added; `renderGoldSheetBody` updates the sheet; `goldSheetMutated = true`. (2) User taps **Done** → `closeGoldSheet` (5282) sees `goldSheetMutated` and calls `renderInventory()`, so the inventory dashboard now paints the phantom `+amount` into the party gold total. (3) User is looking at the inventory tab. (4) The server returns `{ok:false}` (e.g. a lock-timeout or a server-side validation guard the client's `amount > 0` check doesn't cover). (5) `removePending()` deletes the phantom row from `inventoryRows` (state is now correct) but the branch `return`s without `renderInventory()`, and the error message goes to the now-hidden `#goldSheetStatus`. The inventory dashboard **keeps displaying the inflated gold total** until some unrelated event repaints it (a tab switch, another write, or a foreign sync-poll signature change). The user believes the party received gold it did not.

Self-healing and no persisted corruption (the phantom row is never cached — `receivedGold` only calls `cacheInventoryRows` on the ok path, 5435 — so a reload shows the truth), but the on-screen total is silently wrong with the only explanation hidden on a closed sheet. Fix: add `renderInventory();` to the `!res.ok` branch (5428) before the `return`, matching `confirmPayWithReason` (6102) and `receivedGold`'s own throw handler (5447). (`splitGold`'s `!res.ok` branch at 6183 omits `renderInventory()` too, but it is harmless there — `splitGold` mutates only `inventoryResourceLedger`, never `inventoryRows`, so no phantom row survives on the dashboard.)

### 2026-07-09 (run 61) — Sections audited: 9 (Index.html 3001–4500: phone/DPR scaling, swipe gestures, `setCommandMode` nav, command/notes search, combine handlers, inventory load/render + dashboard, collaborative-sync poll, and the full Party-Notes-v2 create/edit/pin/archive/render layer)

> **Stories traced** (happy → failure-at-step → navigate-away → friction, execution-trace + state-machine on each write path): **Create note** / **Edit note** / **Pin note** / **Archive note** / **Review archived notes** (`loadNotes` 4108, `renderNotesList` 4163, `noteCardHtml_` 4134, `openNoteForm` 4249, `saveNoteForm` 4322, `setNoteArchived_` 4444, `archiveNote`/`unarchiveNote` 4472/4478, `toggleNotePin` 4482, `toggleNotesArchived` 4216, `deleteNoteFromForm` 4293, `rehydratePendingNote_` 4310); **View item details** and **Combine duplicate** on their render/trigger side (`renderInventory` 3875, `renderInventoryRowCard` 3935, `renderInventoryDashboard` 3999, `showCombineChoice`/`confirmCombineInventoryItem` 3537/3561); inventory **filter/search** (`renderInventory` filter branch 3898–3933, `handleCommandSearchInput` 3454); and the cross-cutting **Collaborative sync interference** / **Tab-switch in-flight** / **iOS background-foreground** stories where they cross the notes/inventory load paths (`pollSync` 4036, `loadNotes` 4108, `loadInventory` 3749). This section was last swept by run 10 (FINDINGS:3380) and the notes render/sync layer by runs 34/40/50; per protocol I re-confirmed those fixes landed (create-success now calls `renderNotesList` 4415 — run-34 BUG 3489 closed; the `_inFlightNoteWrites` notes-poll defer guard 4052 now exists — run-40 RISK 3475 closed; `toggleNotePin`/`saveNoteForm`-edit now route failures to `#notesStatus` — run-50 IDEA 1088 partially closed, see BUG below) and did NOT re-report them. Dice calc, swipe gesture state machine, and combine rollback re-verified clean (already logged, not re-counted). **Two new findings:**

#### RISK · Index.html:4051 · A foreign notes change is permanently missed when the 20 s poll fires during an in-flight (read-only) `loadNotes` — `syncState.notes.ts` is advanced but the reload silently no-ops

**Story: Collaborative sync interference / Review archived notes — foreign write coincides with a non-write notes load.** The `pollSync` notes branch (4051–4061) unconditionally advances `syncState.notes = res.notes` (4044-equivalent at 4054) in its `else` arm **before** calling the guarded `loadNotes(true)` (4058). But `loadNotes` (4108) opens with `if (notesLoading) return;` (4110) — a re-entrancy guard with **no reschedule**. If a plain read-load is already in flight when the poll fires, `loadNotes(true)` returns immediately without fetching, yet `syncState.notes.ts` has just been set equal to the foreign write's ts. Every subsequent poll then sees `res.notes.ts === syncState.notes.ts` and never reloads again.

Trace: (1) user taps the Notes tab → `loadNotes()` fires `apiGetNotes` (`notesLoading = true`); this read snapshots the sheet at t₀. (2) Another player commits a note write at t₀+0.1 s, bumping `SYNC_NOTES`. (3) The poll fires at t₀+0.2 s while `notesLoading` is still true — note this is a *read*, so `_inFlightNoteWrites` is 0 and the `_inFlightNoteWrites > 0` defer (4052) does **not** apply; control reaches the `else` arm, `syncState.notes` is advanced to the new ts, and `loadNotes(true)` no-ops on the `notesLoading` guard. (4) The original t₀ read returns without the foreign change and repaints `notesData` from stale data. (5) No future poll ever re-detects the change (ts now matches). The foreign note is invisible **indefinitely while the user stays on the Notes tab** — the only recoveries are the user making their own note write (which forces a fresh reload) or leaving and returning *after* the 2-min `NOTES_REVALIDATE_MS` TTL expires (a return within TTL only re-renders from cache, 4112). No data loss, and the window is narrow (poll must land inside the sub-second load), but the failure mode is a silently permanent stale view, which is exactly what the sync layer exists to prevent.

The **inventory branch is immune** to the same shape: `loadInventory(true)` (3749) has no `notesLoading`-style re-entrancy guard — `force = true` bypasses its only early-return (3787), so overlapping inventory fetches are allowed and always refresh. The asymmetry is entirely the notes-only `notesLoading` guard.

Fix: don't advance `syncState.notes` when the reload can't run. Either set `pendingForeignNoteReload = true` and `return` when `loadNotes` is skipped for `notesLoading` (and have the in-flight load's success handler drain it by re-invoking `loadNotes(true)` if the flag is set), or move the `syncState.notes = res.notes` assignment to happen only once `loadNotes` actually initiates a fetch. The cleanest is to have `loadNotes`, on the `notesLoading` early-return, remember a pending force so the current load's completion chains one more fetch.

#### BUG · Index.html:4460,4467 · Archive / Unarchive / form-delete failures report only to the off-screen `#mainStatus`, so the note silently reverts with no visible feedback on the Notes tab  ✅ FIXED (2026-07-13, this session)

**Story: Archive note / Review archived notes (Unarchive) / Edit form → Archive — failure at the server call.** This is the un-closed remainder of the run-50 IDEA (FINDINGS:1088). That IDEA's resolution (FINDINGS:1100, 2026-07-02) added the notes-tab-local `#notesStatus` surface and moved `toggleNotePin` and the **edit** branch of `saveNoteForm` onto `setNotesStatus`, but it left `setNoteArchived_` (4444) — the shared engine behind `archiveNote` (4472), `unarchiveNote` (4478), and `deleteNoteFromForm`'s archive/unarchive toggle (4293) — still calling `setMainStatus` on both its `!ok` (4460) and transport-failure (4467) paths.

`#mainStatus` sits at the very top of `<main>` (line 2214), above both sections; the code itself documents (comment at 3437–3438) that it "scrolls off the top of the page" on the Notes tab, which is the entire reason `#notesStatus` (inside `#notesSection`, line 2473) was created. So when an archive or unarchive round-trip fails, `setNoteArchived_` correctly reverts the optimistic flag and re-renders — the card silently snaps back to its previous list/section — but the accompanying `"Archive failed…"` / `"Unarchive failed…"` message lands in the off-screen `#mainStatus` where a Notes-tab user never sees it. The user watches a note jump to Archived and then jump back with no explanation, unable to tell whether it failed or they mis-tapped — the identical symptom the run-50 fix eliminated for pin and edit. Fix: change the two `setMainStatus(...)` calls in `setNoteArchived_` (4460, 4467) to `setNotesStatus(...)`, matching `toggleNotePin` (4498/4506) and the edit branch of `saveNoteForm` (4361/4371).
...
#### BUG · Index.html:4596 · `loadFallbackCharacterIdentity` failure handler wipes an already-applied cached identity and force-shows the picker on any transient `apiGetMyCharacter` error  ✅ FIXED (2026-07-13, this session)
