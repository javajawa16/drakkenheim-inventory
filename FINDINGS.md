...
#### BUG · Index.html:6231 · "Undo last pay" is unreachable — the only Undo affordance lives in `renderResourceBreakout`, which is never rendered
...
#### RISK · Index.html:7390 · `saveInventoryEdits` omits `_inFlightWrites` (full inventory edit) — only write path in this section still missing the guard
...
#### RISK · Index.html:6936 · `openInventoryDescription` failure handler lacks the `capturedId` navigate-away guard the success handler has
