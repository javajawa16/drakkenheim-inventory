...
### 2026-06-26 (run 59) — Sections audited: 7 (Index.html 1–1500, the CSS `<style>` block; traced cross-cutting modal/scroll-lock + z-index state machinery into Index.html JS as needed)

Stories traced through this section's CSS-driven state: **Create note** (pending/Saving state), **View item details → Give/Sell/Remove** (description sheet + descActionSheet stacking), **Quick-adjust currency/delerium** and **Receive/Sell crystals** (bottom-sheet overlays), **Identity selection**, plus the cross-cutting **navigate-away / iOS background** scenarios for every mobile-sheet overlay.

#### RISK · Index.html:110 · Body scroll-lock relies solely on `overflow:hidden`, unreliable for partial bottom-sheets on iOS GAS webview

**Cross-cutting: navigate-away / iOS for every bottom-sheet flow (description→Sell/Give/Remove, Pay reason, delerium, combine).** The only background-scroll lock is `body.app-modal-open { overflow: hidden; }` (line 110), toggled centrally by `syncModalOpenState()` (3483) whenever any `.mobile-sheet.active` exists. For the *full-screen* sheets (`inset:0`, e.g. note form, identity) this is fine — nothing of the page is reachable. But several sheets are *partial bottom sheets*: `#descActionSheet` (`align-items:flex-end`, panel `max-height:80vh`, lines 1367–1385), `#payReasonSheet`, `#sellItemSheet`, `#giveItemSheet`, and the delerium/combine sheets. They leave a visible, tappable dimmed-backdrop strip (~20vh for descAction) over the inventory list / ledger. On iOS WKWebView (which is what the GAS webview is, rendering at ~980px CSS width), `overflow:hidden` on `<body>` is a well-known *unreliable* scroll lock — momentum/`-webkit-overflow-scrolling` touches on that backdrop area can still scroll the content behind the sheet. There is no JS backstop: the only `touchmove` listener (3264) is scoped to the inventory list's own swipe gesture and guards on `dragging`, so it does not block backdrop scroll. Net effect: a user who opens the Sell/Give/Remove or Pay sheet on a phone and drags on the exposed backdrop may see the list scroll underneath, and on return the scroll position has moved. Confidence is medium — the full-screen fixed overlay (`.mobile-sheet { position:fixed; inset:0 }`, 1352) mitigates it in many cases, so this may not reproduce on every iOS version. Suggested hardening: lock with `position:fixed; width:100%; top:-<savedScrollY>` on `<body>` while a sheet is open (and restore scrollY on close), or add `touch-action:none` / a `touchmove` preventDefault on the sheet backdrop. This is the standard fix for the iOS body-scroll-lock pitfall the audit brief calls out.

#### Note · Index.html:1288,2151 · Pending-note non-clickability is correctly double-enforced; legacy `.notes-note` styling is vestigial

**Story: Create note — pending/in-flight state (navigate-away & double-tap).** Verified the README claim that a saving note card is "dimmed + non-clickable". The *current* Party Notes UI renders `.note-card` and applies `.note-pending` for temp IDs (`NOTE_TEMP_*`, render at 4136–4153); the class sets `opacity:.6; pointer-events:none` (line 2151) AND the renderer omits the `onclick` entirely for pending cards (4142) and the swipe actions (4153). So a card mid-create cannot be tapped to open an edit form against a not-yet-server-side ID — both CSS and JS guard it. The `.notes-note.pending { opacity:.72 }` rule at line 1288 (no `pointer-events`) belongs to the *legacy* `.notes-note` v1 notes UI (the `CAMPAIGN_NOTES_FEED` backend the README lists as still present); it is not the path exercised by the Party Notes story, so the missing `pointer-events:none` there is not reachable from the current flow. Clean trace.

#### Note · Index.html:1352,2536,2765 · Mobile-sheet z-index stacking and scroll-lock centralization are sound

**Stories: View item details → Give/Sell/Remove; all sheet open/close transitions.** Enumerated the overlay z-index ladder: `.mobile-sheet` base = 70 (1352), `descriptionSheet` = 70 (2536), `sellItemSheet`/`giveItemSheet` = 80 (2703/2739), `payReasonSheet` = 80 (CSS 231), `descActionSheet` = 81 (2765). The active description flow opens `descActionSheet` (81) over `descriptionSheet` (70) — correct. The legacy `openGiveItemSheet()` (z80) is only ever opened directly from the description sheet (z70), never from above descActionSheet, so it too renders on top. No sub-sheet can be buried behind a parent. Scroll-lock: every open/close path except the standalone identity picker routes through `syncModalOpenState()` (3483), which recomputes `app-modal-open` from "is any `.mobile-sheet.active`" — so closing a stacked inner sheet correctly *keeps* the lock while an outer sheet remains open (verified: descActionSheet close at 6955 leaves descriptionSheet active → lock stays). The two direct `app-modal-open` add/remove sites are the identity picker (4575/4603), which is a standalone first-open overlay that never stacks with another sheet, so the bypass is safe in practice. Clean trace.
