...
#### BUG · Index.html:5422 · `receivedGold` (and `confirmPayWithReason`/`splitGold`) never increment `_inFlightWrites` — optimistic gold ledger entry gets clobbered or duplicated by a foreign-sync reload
...
#### BUG · Index.html:4421 · Create note silently lost when a foreign notes-sync reload lands during the create round-trip
...
#### BUG · Index.html:5602 · `confirmSellItem` reverts silently on failure — no error feedback to the user
...
#### RISK · Index.html:5413 · Resource write handlers clear the amount/note inputs before the server call and never restore them on failure
...
#### RISK · Index.html:5923 · `confirmSellBatch` omits `_inFlightWrites` and applies its optimistic decrement inside the success handler over possibly-reloaded rows
...
#### RISK · Index.html:4673 · Note pin/archive/delete have no in-flight guard — double-tap pin can desync
...
#### BUG · Index.html:4349 · `pollSync` permanently drops another user's write when it interleaves with my own in-flight write
...
#### RISK · Index.html:4123 · `loadInventory` revalidation overwrites in-flight optimistic state with no `_inFlightWrites` guard
...
#### RISK · Index.html:3449 · `confirmCombineInventoryItem` does not increment `_inFlightWrites`
