...
#### BUG · Index.html:6884,7829 · `editQty`/`sheetEditQty` `readOnly` is set for a rolled-up item but never reset — the quantity field stays locked for every subsequent single-addition edit in the session (with no hint explaining why)
...
#### BUG · Index.html:5428 · `receivedGold` server-error branch drops the phantom Gold row from state but never calls `renderInventory()`, so the inventory dashboard keeps showing gold that was never received
...
#### BUG · Index.html:4460,4467 · Archive / Unarchive / form-delete failures report only to the off-screen `#mainStatus`, so the note silently reverts with no visible feedback on the Notes tab
